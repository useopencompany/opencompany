...
If `node_modules/` is missing, run `npm install`. Non-interactive, safe to run.
...
|---|---|---|
| `create .env.local` | agent | Run `npm run setup` *only if* you can confirm no interactive prompts will fire (i.e. WorkOS already configured upstream). Otherwise treat like next row. |
| `provision WorkOS via npx workos@latest install (interactive)` | **user** | Stop. Tell the user to run `npm run setup` in their terminal — it will offer to launch the WorkOS installer, which auto-provisions a temporary dev environment and writes keys to `.env.local`. No WorkOS account needed for first try. |
| `browser login to Neon (interactive)` | **user** | Stop. Tell the user to run `npx neonctl auth` in their terminal (opens browser). If multi-project, also `npx neonctl set-context --project-id <id>`. |
| `create per-branch DB and apply migrations (non-interactive, safe for agent)` | agent | Run `npm run db:branch:create && npm run db:migrate`. Both are non-interactive. |
...
- The exact next command: `npm run dev`
...
- Don't pipe input to `npm run setup` to skip prompts. The prompts gate WorkOS and Neon flows that genuinely need a human.
- Don't modify `.env.local` directly — let `scripts/setup.mjs` and `scripts/neon-branch.mjs` own it.
- Don't create a Neon branch via `neonctl` directly. Use `npm run db:branch:create` so `.env.local` stays in sync.
- Don't run `npx workos@latest install` yourself in Bash — the installer is interactive and assumes a real terminal.
