...
Kind is **derived from the folder**, not chosen independently: `BrainKindForFolder(folder)`
returns `evidence` iff the folder is `evidence` or starts with `evidence/`
(`BRAIN_EVIDENCE_ZONE = "evidence"`). The database enforces the same invariant with a check
...
`BRAIN_ENTITY_TYPES` — a **closed set**. Types are tags for what a document *is*; folders are
...
Validation: `isValidBrainEntityType` (set membership).
Normalization: `normalizeBrainEntityType` (lowercase, `[^a-z0-9]` → `_`, max 64 chars).
The looser `BRAIN_ENTITY_TYPE_PATTERN` (`^[a-z][a-z0-9_]{0,63}$`) only bounds the shape;
the closed set above is what validators actually accept.

## Status lifecycle

`BRAIN_STATUS_VALUES = ["draft", "active", "archived", "merged"]`

| Status | Meaning |
| --- | --- |
| `draft` | Captured but not yet curated. Chat captures land in `inbox/` as drafts. |
| `active` | Curated, trusted knowledge. **Promotion to `active` requires the compiled truth to cite at least one valid `[[evidence:...]]` or `[[source:...]]` link** (enforced by `opencompany-brain set`). |
...
`DEFAULT_BRAIN_FOLDERS` — the 10 folders seeded per Brain. `inbox`, `people`, `companies`,
...
Folder paths match `BRAIN_FOLDER_PATTERN`: lowercase `a-z0-9-` segments separated by `/`,
...
| Thing | Pattern | Notes |
| --- | --- | --- |
| Document id (`brain_id`) | `^[a-z0-9][a-z0-9-]{0,79}$` | Slug, max 80 chars. `normalizeBrainId` lowercases, strips quotes, collapses non-alphanumerics to `-`. |
| Evidence id | `^ev-[a-z0-9][a-z0-9-]{0,76}$` | Evidence records and timeline entries carry `ev-*` ids. |
| Source ref | `^[a-z0-9][a-z0-9-]{0,63}:[^\s[\]|]+$`, ≤ 256 chars | `provider:id`. The id part may itself contain colons/slashes: `jamie:meeting:calendar_event_123` is provider `jamie`, id `meeting:calendar_event_123`. Parse with `parseBrainSourceRef`. |
| Relation type | `^[a-z][a-z0-9_]*$` | Default `related`. Free-form beyond the pattern (e.g. `owner`, `about`, `works_at`). |
| Inline link | `[[kind:target]]` or `[[kind:target\|Label]]` | `kind` is `page`, `evidence`, or `source` (`BrainInlineLinkKind`, `inline-links.ts`). Bare `[[target]]` is a legacy page link. |
...
(`BRAIN_TRUTH_HEADING`, `BRAIN_TIMELINE_HEADING` in `document.ts`):
...
- **Frontmatter** — the full field contract is `BrainFrontmatter` in `schema.ts`: required
  `id`, `folder`, `kind`, `type`, `status`, `createdAt`, `updatedAt`, `relations`; optional
  `title`, `description`, `aliases`, `sources`, `mergedInto`, `legacyKeys`.
- **Compiled truth** — the current state of knowledge, rewritten in place (`opencompany-brain rewrite`).
...
(`BrainDocumentFormat`). A file-backed document is **one row, one folder entry, one artifact**
...
All in the `goat` Postgres schema, defined in `packages/db/src/product-schema.ts`. Everything
brain-scoped carries a `brain_ref`.

| Table | Drizzle export | Holds |
| --- | --- | --- |
| `brains` | `Brains` | Brain metadata: workspace, name, slug, visibility, creator. |
| `brain_members` | `BrainMembers` | Per-brain membership/access. |
| `brain_folders` | `BrainFolders` | Folder rows per brain (`source: system \| custom`). |
| `brain_documents` | `BrainDocuments` | The documents: content, folder_path, kind, entity_type, status, sources, relations, format. Check constraints enforce the kind↔folder zone rule and the enum values above. |
| `brain_timeline_entries` | `BrainTimelineEntries` | Append-only timeline rows (evidence_id, at, source_ref, summary, detail). |
| `brain_edges` | `BrainEdges` | Derived graph edges from relations and wiki-links (`source_kind: relation \| wiki_link`). |
| `brain_document_versions` | `BrainDocumentVersions` | Version history (`operation: overwrite \| delete`). |
