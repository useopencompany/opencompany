...
### Hot memory (`agent/user.md`, `agent/memory.md`)

Every session injects two small "hot memory" files from the agent folder directly into the
system prompt, so the agent carries durable context into each session without having to retrieve
it:

- **`agent/user.md`** — who the user is: identity, preferences, communication style, goals.
- **`agent/memory.md`** — environment, conventions, and durable lessons / workflows.

Both are auto-created empty on first use and edited by the agent with ordinary file tools as it
learns. They are deliberately small: each is hard-capped at ~3 KB when injected (content beyond
the cap is truncated with a visible marker), so larger or long-tail durable facts belong in
structured memory (`agent/memory/`, via the `memory` tool) or other private `agent/` files
instead. Because the prompt is built at session start, edits take effect on the **next** session
(a frozen snapshot per session). This applies to any agent that has these files — it is not
specific to the personal agent.
...
The after-session run is not a visible chat turn. It reuses the agent loop, can use configured tools, and should capture anything worth carrying forward into the hot-memory files (`agent/user.md` for durable facts about the user, `agent/memory.md` for durable environment/workflow learnings) when there is useful long-lived context to preserve — keeping both tight given the ~3 KB cap. Use Brain only for shared company knowledge.
