...
The legacy web app and opencompany send to separate PostHog projects. The shared package only reuses the
...
- `packages/analytics/src/goat-events.ts` is the opencompany event registry.
...
opencompany's product events are:
...
There is no separate `chat_started` event because it would double-count the first message. opencompany
...
opencompany identifies people with the internal WorkOS user ID. It sets workspace ID, email, first name,
...
Store the opencompany project's API host and project token in:

- Infisical `prod` + `/goat`, synced to the opencompany Vercel project
...
- Infisical `dev` + `/web` when local opencompany analytics are needed
...
With opencompany PostHog values missing, exercise signup, app load, chat, and integration connection and
...
With the real opencompany project values, confirm the explicit events exercised by the app, shared Stripe
webhook, and runner appear in the opencompany project, and that none appear in the legacy web project.
