...
| `NEXT_PUBLIC_GOAT_POSTHOG_TOKEN` / `NEXT_PUBLIC_GOAT_POSTHOG_HOST` | Yes | Enables product analytics in the dedicated Goat PostHog project. Store the same Goat-project values in Infisical `prod` + `/goat`, `/runner` (Brain ingestion worker), and temporarily `/web` for the Stripe cutover rollback route; do not reuse the legacy web project token. |
