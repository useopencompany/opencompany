...
| `GOAT_MAIN_CHAT_INTEGRATION_TOOLS_DISABLED` | No | Global kill switch for the connected-tools-in-main-chat beta (Linear/GitHub read tools in foreground chat). Set `true` to disable the feature for every user without a deploy rollback; the per-user beta toggle in Settings → Preferences then reads as off and integration tools are not exposed to the model. Unset/`false` means the per-user beta flag decides. |
