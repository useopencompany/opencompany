...
3. Add a `personal_network` action resolver in `@opencompany/core` and include it in `resolveActionCatalog` when imported contacts exist.
...
opencompany already exposes a managed `linkedin` source for public LinkedIn research in `apps/app/lib/capabilities/catalog.ts`. That source explicitly does not connect to the user's account, so it should remain separate from this feature. The new surface should be a first-party `personal_network` connected/internal source backed by user-imported data.
