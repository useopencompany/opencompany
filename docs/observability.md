...
Better Stack/Sentry-compatible DSNs are required for the production web, API, and runner releases so
uncaught server renders, browser error-boundary failures, and server-side `captureException` calls
reach the error tracker. Web uses `@sentry/nextjs`; the API and runner install the shared
`@opencompany/observability/sentry-bun` reporter at boot, which tags every event with the service name
so one Better Stack application can be filtered per runtime. Without a DSN, `captureException` only
writes a local structured log with `observability_reporter: "local"`, and the Bun services log
`opencompany.error_reporting_disabled` once at startup so that mode is visible. `bun run setup` does
not mirror the DSNs into local development; set them in `.env.override.local` only when testing error
capture on purpose, so local runs do not pollute the tracker with `development` events.

API request failures are reported from the Hono `onError` handler, which is where every handler
error lands. Expected 4xx domain outcomes (`ApiError`/`CoreError` such as not found, forbidden,
rate limited, or validation failures) are not defects and are not reported; 5xx responses and errors
of unknown shape are.

Web error events retain the route, method, release, digest, stack, safe tags,
