...
capture on purpose, so local runs do not pollute the tracker with `development` events. Setup also
removes DSNs generated by older versions from root and web `.env.local` files while preserving
developer-owned values in `.env.override.local`.
