...
opencompany main chat can use a persistent, session-keyed Vercel Sandbox for rendered
public-web browsing. Browser tools are available to every default chat;
...
For local development, link the opencompany Vercel project and run `vercel env pull`;
...
are served through an authenticated opencompany route and are never exposed as direct
Blob URLs.
...
`CHAT_SANDBOX_IMAGE` in Infisical `prod` + `/goat`, sync it to the opencompany
Vercel project, and verify Sandbox authentication and Blob storage.

If the image variable is absent, opencompany creates a stock Node 24 sandbox and
...
Start a new default chat and ask:
