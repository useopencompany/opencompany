...
- **Google (Gmail + Calendar + Drive) integration bootstrap only:** create an OAuth client in the
  ...
  screen, Gmail's `gmail.readonly` and Drive's `drive` scopes are **restricted**, and Calendar is
  **sensitive**: for external/production users Google requires app verification (and restricted
  scopes can require a CASA security assessment). Until verification, add yourself as a **test user**
  on the consent screen — the flow works fully for test users.
