...
OpenCompany has one product generation: Goat.

## Runtime boundaries

- `apps/goat` owns the Next.js UI, authenticated server actions, public integration callbacks and
...
The detailed turn and message flow is maintained in [the Goat system map](../apps/goat/docs/README.md).
...
migration history remains authoritative and is never rewritten. Goat collections consume authorized
...
WorkOS sessions authenticate Goat. OAuth state and integration credentials are signed/encrypted at
