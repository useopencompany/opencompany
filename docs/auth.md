...
Goat uses WorkOS AuthKit. `apps/goat/lib/workos.ts` owns the canonical app URL and redirect URI;
`apps/goat/lib/auth.ts` maps the WorkOS identity to Goat users and workspaces.
...
The runner does not accept browser sessions. Private Goat-to-runner requests use
`RUNNER_INTERNAL_TOKEN`, and browser-reachable runner transports use scoped signed tickets plus
`RUNNER_ALLOWED_ORIGINS`.

Goat's external integrations use provider-specific OAuth state secrets and encrypted credential
