...
CREATE TABLE "goat"."brain_source_items" (
	...
	CONSTRAINT "goat_brain_source_items_source_provider_check" CHECK ("goat"."brain_source_items"."source_provider" IN ('jamie')),
...
CREATE TABLE "goat"."brain_ingest_jobs" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "goat_brain_ingest_jobs_source_provider_check" CHECK ("goat"."brain_ingest_jobs"."source_provider" IN ('jamie')),
...
ALTER TABLE "goat"."brain_source_items" ADD CONSTRAINT "goat_brain_source_items_integration_user_provider_fk" FOREIGN KEY ("integration_id","user_workos_id","source_provider") REFERENCES "goat"."integrations"("id","user_workos_id","provider") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE UNIQUE INDEX "goat_brain_source_items_connection_external_hash_idx" ON "goat"."brain_source_items" USING btree ("user_workos_id","source_provider","source_connection_id","source_type","external_id","content_hash");--> statement-breakpoint
CREATE INDEX "goat_brain_source_items_user_provider_occurred_idx" ON "goat"."brain_source_items" USING btree ("user_workos_id","source_provider","occurred_at");--> statement-breakpoint
