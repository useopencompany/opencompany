  ...
  return (
    <Link
      ...
      className={`group flex w-full items-center gap-2.5 rounded-md px-3 py-[6px] text-[13px] transition-colors duration-150 focus:outline-none focus-visible:ring-1 focus-visible:ring-ink/20 ${
        active ? "bg-ink text-[#f7f7f5]" : "text-ink/70 hover:bg-[#e8e8e5] hover:text-ink"
      }`}
    >
      <FileText
        size={13}
        strokeWidth={1.75}
        className={active ? "text-[#f7f7f5]/70" : "text-ink/35 group-hover:text-ink/60"}
      />
...
function DocsSidebar({ tree, activeUrl }: { tree: Root; activeUrl: string }) {
  return (
    <aside className="relative hidden h-screen w-[240px] shrink-0 overflow-hidden bg-[#f0f0ee] after:absolute after:inset-y-0 after:right-0 after:w-px after:bg-[#e2e2de] md:block">
      <div className="flex h-full w-[240px] flex-col">
        {/* Sidebar header */}
        <div className="flex items-center gap-2.5 border-b border-[#e2e2de] px-4 py-3.5">
          <Link
            href="/"
            prefetch={false}
          ...
          >
            <span
              aria-hidden
              className="flex h-5 w-5 shrink-0 items-center justify-center rounded-[5px] bg-ink text-[10px] font-bold text-[#f7f7f5]"
            >
              oc
            </span>
            opencompany
          </Link>
          <span className="ml-auto shrink-0 rounded-[4px] bg-[#e6e6e3] px-1.5 py-0.5 text-[10px] font-medium tracking-tight text-ink-muted">
            docs
          </span>
        </div>
        ...
        {/* Footer */}
        <div className="mt-auto border-t border-[#e2e2de] px-4 py-3">
          <Link
            href="/"
            prefetch={false}
...
function DocsTopBar({ title }: { title: string }) {
  return (
    <header className="sticky top-0 z-20 border-b border-[#e6e6e3] bg-[#f7f7f5]/95 backdrop-blur-md">
      <div className="flex h-[52px] items-center gap-3 px-5">
        ...
        >
          <span
            aria-hidden
            className="flex h-5 w-5 items-center justify-center rounded-[5px] bg-ink text-[9.5px] font-bold text-[#f7f7f5]"
          >
            oc
          </span>
          Docs
        ...
        <div className="ml-auto flex items-center gap-1.5">
          <Link
            href="/changelog"
            prefetch={false}
            className="inline-flex items-center gap-1 rounded-md px-2.5 py-1.5 text-[12px] text-ink-muted transition-colors hover:bg-[#eeeeeb] hover:text-ink"
          >
            Changelog
...
function DocsFooterNav({ tree, activeUrl }: { tree: Root; activeUrl: string }) {
  ...
  return (
    <nav
      className="mt-12 flex items-center justify-between border-t border-[#ececea] pt-6"
    ...
    >
      <div className="min-w-0 flex-1">
        {prev && (
          <Link
            href={prev.url}
            className="group inline-flex flex-col gap-0.5 rounded-lg border border-[#e6e6e3] px-4 py-3 transition-colors hover:border-[#d8d8d3] hover:bg-[#fafaf8]"
          >
      ...
      <div className="min-w-0 flex-1 text-right">
        {next && (
          <Link
            href={next.url}
            className="group inline-flex flex-col items-end gap-0.5 rounded-lg border border-[#e6e6e3] px-4 py-3 transition-colors hover:border-[#d8d8d3] hover:bg-[#fafaf8]"
          >
            <span className="text-[11px] font-medium uppercase tracking-[0.06em] text-ink-subtle transition-colors group-hover:text-ink-muted">
              Next →
            </span>
...
export default function DocsView({ title, description, url, tree, toc, body: MDX }: DocsViewProps) {
  return (
    <div className="flex min-h-screen w-screen bg-[#f7f7f5]">
        ...
        <div className="flex min-h-0 flex-1">
          ...
          <main className="min-w-0 flex-1 overflow-y-auto">
            <article className="mx-auto w-full max-w-[720px] px-6 pb-20 pt-10 sm:px-10 lg:px-12">
              ...
              {/* Divider */}
              <div className="mb-8 h-px w-full bg-[#e6e6e3]" />
