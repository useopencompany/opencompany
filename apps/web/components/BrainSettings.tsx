...
import {
  ...
  getBrainIntelligenceAction,
  setBrainAccessAction,
  setBrainEnrichmentAction,
  setBrainIntelligenceAction,
      ...
      {tab === "overview" ? (
        ...
        <div className="flex w-full max-w-[640px] flex-col gap-8">
          ...
          <SettingsSection title="Company context">
            <BrainImport brainRef={brain.id} compact />
          </SettingsSection>
        </div>
      )}
    </div>
...
function BrainAccessSection({
  ...
  useEffect(() => {
    let cancelled = false;
    void getBrainAccessDetailsAction(brain.id).then((details) => {
  ...
  const save = () => {
    startTransition(async () => {
      const result = await setBrainAccessAction({
        brainRef: brain.id,
        visibility,
...
function IntelligenceSection({ brainRef }: { brainRef: string }) {
  ...
  useEffect(() => {
    let cancelled = false;
    void getBrainIntelligenceAction(brainRef)
      .then((details) => {
        if (cancelled) return;
        if (!details) {
  ...
  const select = (next: "basic" | "frontier") => {
    ...
    startTransition(async () => {
      const result = await setBrainIntelligenceAction({ brainRef, intelligence: next });
      if (!result.ok) {
        setState((current) =>
  ...
  return (
    <div className="flex flex-col gap-2">
      <label className="flex cursor-pointer items-start gap-3 rounded-md border border-ink/10 p-3">
        <input
          type="radio"
          name={`opencompany-brain-intelligence-${brainRef}`}
...
function EnrichmentSection({ brainRef }: { brainRef: string }) {
  ...
  useEffect(() => {
    let cancelled = false;
    void getBrainEnrichmentEnabledAction(brainRef)
      .then((details) => {
        if (cancelled) return;
        if (!details) {
  ...
  const toggle = () => {
    ...
    startTransition(async () => {
      const result = await setBrainEnrichmentAction({
        brainRef,
        enabled: next,
      });
      if (!result.ok) {
