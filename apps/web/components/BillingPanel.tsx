...
export type BillingPanelData = BillingOverviewDto;
...
export function BillingPanel({
  ...
  const topUpCustom = () => {
    ...
    run(() => createCreditTopUpAction(cents, crypto.randomUUID()));
  };
  ...
  const saveAutoRefill = (enabled: boolean) => {
    ...
    startTransition(async () => {
      const result = await setAutoRefillAction(
          ...
          {data.isAdmin ? (
            data.plan === "pro" || hasManageableSubscription ? (
              <button
                type="button"
                disabled={isPending}
                onClick={() => run(() => createBillingPortalAction(crypto.randomUUID()))}
              ...
              <button
                type="button"
                disabled={isPending}
                onClick={() => run(() => createProCheckoutAction(crypto.randomUUID()))}
            ...
            <div className="flex flex-wrap items-center gap-2">
              {data.topUpAmountsCents.map((amountCents) => (
                <button
                  ...
                  disabled={isPending}
                  onClick={() =>
                    run(() => createCreditTopUpAction(amountCents, crypto.randomUUID()))
                  }
