...
import {
  createJamieWebhookEndpointAction,
  saveJamieWebhookKeyAction,
} from "@/lib/integrations/jamie-events-actions";

// Jamie has no webhook-management API, so the connection is made in Jamie's own settings: create
// the endpoint here, copy its URL into Jamie, and bring the key Jamie mints back.
export function JamieEventsSetup({ initialState }: { initialState: JamieEventsProviderState }) {
  ...
  function run(action: () => Promise<Awaited<ReturnType<typeof saveJamieWebhookKeyAction>>>) {
    setError(null);
      ...
      {state.webhookUrl ? (
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <p className="text-[12.5px] leading-5 text-ink-subtle">
            Create an endpoint to get the URL Jamie should send completed meetings to.
          </p>
          <Button
            size="sm"
            disabled={isPending}
            onClick={() => run(createJamieWebhookEndpointAction)}
          >
      ...
      {state.webhookUrl ? (
        <>
          <ol className="list-decimal space-y-1 pl-5 text-[12.5px] leading-5 text-ink-subtle">
            ...
            <li>
              Choose a personal webhook for your own meetings, or a workspace webhook for everyone
              in your Jamie workspace.
            </li>
            <li>
