...
import { createBrainAction, switchBrainAction } from "@/lib/workspace-actions";

export function BrainSwitcher() {
  const { brains, activeBrain, workspace } = useAppData();
  ...
  const switchBrain = (brainRef: string) => {
    const href = brainHref(brainRef);
    startTransition(async () => {
      const result = await switchBrainAction(brainRef);
      if (!result.ok) {
...
function CreateBrainDialog({ onClose }: { onClose: () => void }) {
  ...
  const submit = () => {
    ...
    startTransition(async () => {
      const result = await createBrainAction({ name, visibility });
      if (!result.ok) {
        toast.error(result.error);
        return;
      }
      onClose();
      if (result.brainRef) router.push(brainHref(result.brainRef));
      else router.refresh();
    });
  };
...
function brainHref(brainRef: string) {
