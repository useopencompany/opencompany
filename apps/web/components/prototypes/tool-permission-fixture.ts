import type { PrototypeGroup, PrototypeTool } from "./tool-permission-model";

/**
 * The real Gmail plugin surface: the 17 tools registered in
 * packages/agent/src/integrations/gmail-mcp-server.ts, in the three capability groups the settings
 * page renders today. Real data matters here — the whole question is whether the interaction
 * survives a group with nine tools in it.
 */
const READ_GMAIL: PrototypeGroup = {
  id: "query",
  label: "Read Gmail",
...
const CREATE_DRAFTS: PrototypeGroup = {
  id: "draft",
  label: "Create drafts",
...
/** The everyday label write that today's single "Always allow" over-grants into eight siblings. */
const LABEL_THREAD: PrototypeTool = {
  id: "gmail:label_thread",
  name: "Label thread",
  description: "Apply one or more Gmail labels to every message in a thread.",
};

const ORGANIZE_GMAIL: PrototypeGroup = {
  id: "write",
  label: "Organize Gmail",
  description: "Add or remove labels, create labels, move mail to trash, and mark or unmark spam.",
  defaultMode: "ask",
  tools: [
    LABEL_THREAD,
    {
...
export const GMAIL_GROUPS: PrototypeGroup[] = [READ_GMAIL, CREATE_DRAFTS, ORGANIZE_GMAIL];

/** The approval the prototype replays, and the group it would widen today. */
export const APPROVAL_GROUP = ORGANIZE_GMAIL;
export const APPROVAL_TOOL = LABEL_THREAD;
