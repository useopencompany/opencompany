...
function MentionPill({ item }: { item: AgentMentionItem }) {
  ...
  return (
    <span className="inline-flex h-[18px] items-center gap-1 rounded-[5px] border border-border bg-surface-muted px-1.5 text-[10.5px] font-medium text-ink/85">
...
function MarkdownPreviewThumb({ mentions }: { mentions: AgentMentionItem[] }) {
  ...
  return (
    <div className="relative h-[88px] w-[132px] shrink-0">
      <div className="absolute left-2 top-2 h-[78px] w-[122px] rounded-md border border-border bg-surface" />
      <div className="relative h-[78px] w-[122px] overflow-hidden rounded-md border border-border bg-surface p-2 shadow-[0_1px_2px_rgba(15,15,15,0.04)]">
        ...
        <div className="mt-1.5 flex flex-wrap gap-1">
          ...
          {overflow > 0 && (
            <span className="inline-flex h-[18px] items-center rounded-[5px] bg-surface-subtle px-1.5 text-[10px] font-medium text-ink-muted">
              +{overflow}
            </span>
          )}
        </div>
...
function AgentRow({ agent, workspaceId }: { agent: AgentListItemPayload; workspaceId: string }) {
  ...
  return (
    <Link
      ...
      className="group relative flex w-full items-center gap-4 rounded-lg px-2 py-2 text-left transition-colors duration-150 hover:bg-surface-subtle/70 focus:outline-none focus-visible:ring-1 focus-visible:ring-ink/15"
    >
...
function NewAgentButton({ label = "New agent" }: { label?: string }) {
  ...
  return (
    <button
      ...
      onClick={handleClick}
      disabled={isPending}
      className="inline-flex h-8 shrink-0 items-center gap-1.5 whitespace-nowrap rounded-md border border-ink/10 bg-ink px-3 pr-3.5 text-[12.5px] font-medium text-canvas shadow-[0_1px_2px_rgba(0,0,0,0.16)] transition-colors duration-150 hover:bg-ink/85 disabled:hover:bg-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-ink/15 disabled:cursor-not-allowed disabled:opacity-60"
    >
      {isPending ? (
