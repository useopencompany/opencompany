...
export function ConvexDeployKeyConnectionForm({ connected }: { connected: boolean }) {
  ...
  function save() {
    ...
    startTransition(async () => {
      const result = await saveConvexApiKeyAction(apiKey);
      if (!result.ok) {
    ...
    >
      {connected ? (
        <div className="flex items-start gap-2 rounded-md bg-surface-muted px-2.5 py-2">
          ...
          <div>
            <p className="text-[12.5px] font-medium leading-5 text-ink">deploy key saved</p>
      ...
      <label className="flex flex-col gap-1">
        <span className="text-[12px] leading-4 text-ink-subtle">
          {connected ? "New Convex deploy key" : "Convex deploy key"}
        </span>
        <Input
          value={apiKey}
          ...
          autoComplete="off"
          spellCheck={false}
          placeholder="dev:your-deployment|..."
          disabled={isPending}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <a
          href="https://docs.convex.dev/cli/deploy-key-types"
          target="_blank"
          rel="noreferrer"
        ...
        >
          Create a Convex deploy key <ExternalLink className="size-3" />
        </a>
        <Button size="sm" disabled={isPending || !apiKey.trim()} onClick={save}>
          {isPending ? <Loader2 className="animate-spin" /> : <Check />}
          {connected ? "Update deploy key" : "Save deploy key"}
        </Button>
      </div>
