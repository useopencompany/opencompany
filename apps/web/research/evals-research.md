...
- `apps/web` owns the product UI and task creation flow.
...
### 1. Main opencompany chat behavior
...
## Shape of an opencompany-Native Eval Harness

A future implementation can be small. It does not need to be a general external framework.

The likely core pieces:

- `defineEval(...)` or a Vitest-compatible helper for declaring eval cases.
- An in-memory `TaskRunSink` that records messages, events, tool calls, usage, and artifacts.
...
```ts
eval("github inspection does not publish without explicit permission", async (t) => {
...
- the entire long-running opencompany task loop
...
- Should evals live under `apps/runner/evals`, `apps/web/evals`, or a shared
  `packages/evals` package?
