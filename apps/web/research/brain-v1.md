# opencompany Brain v1 Best-Practice Scope

Status: v1 implementation contract and research notes.

This document defines the opencompany Brain v1 product and technical contract for a durable, trustworthy,
agent-readable brain that opencompany can use across chat, tasks, research, and connected-account
workflows.

Reference inputs:

- `apps/web/research/gbrain-deep-dive.md`
...
opencompany Brain should be the user's durable private knowledge layer for everything opencompany learns, creates,
...
opencompany Brain needs a machine-readable resolver, even if the first version is Markdown. The resolver is
...
```yaml
api_version: opencompany-brain-schema-pack-v1
name: opencompany-default
version: 0.1.0
types:
  - name: person
    primitive: entity
...
opencompany Brain needs identity resolution from day one, even if the first version is simple.
...
opencompany Brain retrieval should combine multiple signals:
...
opencompany Brain should have a small tool/CLI surface that agents can use safely.
...
We should build a small opencompany Brain eval before relying on intuition.
...
opencompany Brain v1 should be considered real when:
...
- Treating opencompany Brain as a general document management system.
...
1. Should opencompany Brain's first source of truth be Markdown files, database rows with Markdown content,
...
9. How visible should Memory be in the opencompany UI?
