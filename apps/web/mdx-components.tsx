...
export function getMDXComponents(components?: MDXComponents): MDXComponents {
  return {
    ...
    a: ({ children, ...props }) => (
      <a
        className="font-medium text-ink underline decoration-border-strong underline-offset-[3px] transition-colors hover:decoration-ink"
        {...props}
      >
        {children}
      </a>
    ...
    code: ({ children, ...props }) => (
      <code
        className="rounded-[4px] border border-border bg-surface-muted px-[5px] py-[2px] font-mono text-[12px] leading-none text-ink"
        {...props}
      >
        {children}
      </code>
    ...
    pre: ({ children, ...props }) => (
      <pre
        className="mt-5 overflow-x-auto rounded-xl border border-border bg-surface-muted px-5 py-4 text-[12.5px] leading-[1.65] text-ink shadow-[0_1px_3px_rgba(0,0,0,0.04),0_1px_0_rgba(0,0,0,0.02)]"
        {...props}
      >
        {children}
      </pre>
    ...
    blockquote: ({ children, ...props }) => (
      <blockquote
        className="mt-5 rounded-r-lg border-l-[3px] border-border-strong bg-surface-muted px-5 py-3 text-[13.5px] leading-[1.7] text-ink-muted"
        {...props}
      >
        {children}
      </blockquote>
    ...
    thead: ({ children, ...props }) => (
      <thead className="border-b border-border bg-surface-muted" {...props}>
        {children}
      </thead>
    ),
