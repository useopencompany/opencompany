...
vi.mock("@opencompany/agent/brain-cli", () => brainCliMock);
vi.mock("@opencompany/agent/brain-capture", () => captureMock);

import { registerBrainTools } from "@opencompany/agent/mcp-server";
...
describe("opencompany MCP tools", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("brain still accepts the raw command/flags shape", async () => {
    const tools = registerTools();
