...
describe("engine auth command adapters", () => {
  ...
  it("loads Claude Code status through GET /v1/engine-auth/claude-code", async () => {
    ...
    await expect(loadCurrentGoatClaudeCodeAuthSettings()).resolves.toEqual({
      status: "connected",
      statusReason: null,
      lastValidatedAt: "2026-08-12T10:00:00.000Z",
      lastRotatedAt: null,
  ...
  it("saves the Claude Code token through PUT and revalidates settings", async () => {
    ...
    await expect(saveGoatClaudeCodeToken("sk-ant-oat01-token")).resolves.toEqual({ ok: true });
  ...
  it("passes the validator's error copy through unchanged", async () => {
    ...
    await expect(saveGoatClaudeCodeToken("")).resolves.toEqual({
      ok: false,
  ...
  it("disconnects Claude Code through DELETE", async () => {
    ...
    await expect(disconnectGoatClaudeCodeAuth()).resolves.toEqual({ ok: true });
  ...
  it("keeps the by-user connectivity reads on the db while legacy callers remain", async () => {
    ...
    await expect(isGoatClaudeCodeConnectedForUser("user_1")).resolves.toBe(true);
    expect(loadGoatClaudeCodeAuthStatus).toHaveBeenCalledWith({
      db: { db: "sentinel" },
      userWorkosId: "user_1",
    });

    await expect(isGoatCodexConnectedForUser("user_1")).resolves.toBe(false);
  ...
  it("starts a Codex device flow through POST /v1/engine-auth/codex/device", async () => {
    ...
    await expect(startGoatCodexDeviceAuth()).resolves.toEqual({ ok: true, flow });
  ...
  it("requires a flow id before polling and revalidates on completion", async () => {
    await expect(pollGoatCodexDeviceAuth("   ")).resolves.toEqual({
      ok: false,
    ...
    await expect(pollGoatCodexDeviceAuth("gcodf_1")).resolves.toEqual({ ok: true, flow });
  ...
  it("surfaces runner failure copy from the API on Codex starts", async () => {
    ...
    await expect(startGoatCodexDeviceAuth()).resolves.toEqual({
      ok: false,
  ...
  it("disconnects Codex through DELETE", async () => {
    const requests = stubApi(() => Response.json({ data: { deleted: true }, meta }));
    await expect(disconnectGoatCodexAuth()).resolves.toEqual({ ok: true });
    expect(requests[0]?.method).toBe("DELETE");
  ...
  it("loads the workspace Infisical status through GET", async () => {
    ...
    await expect(loadCurrentGoatInfisicalAuthSettings()).resolves.toMatchObject({
      status: "connected",
      host: "https://eu.infisical.com",
    });
  ...
  it("passes the admin-gate copy through on Infisical starts", async () => {
    ...
    await expect(startGoatInfisicalAuth({ host: "https://app.infisical.com" })).resolves.toEqual({
      ok: false,
  ...
  it("starts an Infisical flow with the selected host", async () => {
    ...
    const requests = stubApi(() => Response.json({ data: { flow }, meta }, { status: 201 }));
    await expect(startGoatInfisicalAuth({ host: "https://eu.infisical.com" })).resolves.toEqual({
      ok: true,
      flow,
    });
  ...
  it("keeps the retired browser-token guards before calling the API", async () => {
    ...
    await expect(
      completeGoatInfisicalAuth({ flowId: "ginff_1", browserToken: "   " }),
    ).resolves.toEqual({ ok: false, error: "Paste the browser token from Infisical." });
    await expect(
      completeGoatInfisicalAuth({ flowId: "ginff_1", browserToken: "a".repeat(64 * 1024 + 1) }),
  ...
  it("completes an Infisical flow and revalidates settings on completion", async () => {
    ...
    const requests = stubApi(() => Response.json({ data: { flow }, meta }));
    await expect(
      completeGoatInfisicalAuth({ flowId: " ginff_1 ", browserToken: " browser-token " }),
  ...
  it("returns the disconnect gate copy instead of throwing for Infisical", async () => {
    stubApi(() => errorEnvelope("Only workspace admins can manage Infisical.", 403));
    await expect(disconnectGoatInfisicalAuth()).resolves.toEqual({
      ok: false,
      error: "Only workspace admins can manage Infisical.",
    });
    expect(revalidatePath).not.toHaveBeenCalled();
    ...
    const requests = stubApi(() => Response.json({ data: { deleted: true }, meta }));
    await expect(disconnectGoatInfisicalAuth()).resolves.toEqual({ ok: true });
    expect(requests[0]?.method).toBe("DELETE");
    expect(revalidatePath).toHaveBeenCalledWith("/settings");
  });
});
