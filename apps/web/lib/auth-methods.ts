...
export function safeReturnPathname(value: unknown): string {
  ...
  const baseUrl = new URL("https://opencompany.invalid");
  try {
...
export async function recordLastAuthMethod(method: AuthenticationResponse["authenticationMethod"]) {
  const resolvedMethod = toAuthMethod(method);
  if (!resolvedMethod) return;
  const cookieStore = await cookies();
  cookieStore.set(LAST_AUTH_METHOD_COOKIE, resolvedMethod, {
    path: "/",
    sameSite: "lax",
    maxAge: 60 * 60 * 24 * 365,
  });
