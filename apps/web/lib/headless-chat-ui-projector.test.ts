...
describe("HeadlessChatUiProjector", () => {
  it("keeps post-tool text in a new ordered text segment", () => {
    ...
    expect(chunks).toEqual([
      ...
      { type: "tool-output-available", toolCallId: "tool_1", output: { ok: true } },
  ...
  it("continues with a fresh segment after reconnect and closes it before cancellation", () => {
    const projector = new HeadlessChatUiProjector("message_assistant_1", "Existing", 4, ["tool_1"]);
    const chunks = project(projector, [
    ...
    expect(chunks).toEqual([
      ...
      { type: "tool-approval-request", approvalId: "approval_1", toolCallId: "tool_1" },
