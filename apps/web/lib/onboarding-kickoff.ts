...
3. Save several focused findings rather than one large dump:
   - For Gmail or Linear findings, call save_to_brain with the action result's sourceRef and integrationId, omitting content so background ingestion can hydrate the complete provider source. Add only a short fallbackContent if useful.
   - For Slack findings, include the exact durable content to save and its sourceRef; Slack is accessed through the plugin and is not background-ingested.
