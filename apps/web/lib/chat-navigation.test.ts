...
describe("parseOptimisticGoatChatSessionId", () => {
  it("accepts browser-reserved Goat chat UUIDs", () => {
    expect(
      parseOptimisticGoatChatSessionId("goat_chat_123e4567-e89b-42d3-a456-426614174000"),
    ).toEqual({
      ok: true,
  ...
  it.each(["chat_123", "goat_chat_not-a-uuid", 123])("rejects invalid ids: %s", (value) => {
    expect(parseOptimisticGoatChatSessionId(value)).toEqual({
      ok: false,
