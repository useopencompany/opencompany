...
describe("opencompany billing API adapters", () => {
  const topUp = vi.fn();
  ...
  it("forwards an explicit idempotency key for every billing command", async () => {
    await expect(createCreditTopUpAction(1_000, "topup-1")).rejects.toThrow("NEXT_REDIRECT");
    await expect(createProCheckoutAction("subscription-1")).rejects.toThrow("NEXT_REDIRECT");
    await expect(createBillingPortalAction("portal-1")).rejects.toThrow("NEXT_REDIRECT");
    await expect(
      setAutoRefillAction({ enabled: true, amountCents: 2_000 }, "refill-1"),
  ...
  it("returns the canonical API error without touching Stripe in web", async () => {
    ...
    await expect(createCreditTopUpAction(1_000, "topup-2")).resolves.toEqual({
      ok: false,
      error: "Admin only.",
    });
