...
Goat saves the scopes reported by Attio when the key is validated. Chat can search and inspect standard people, companies, and deals; discover lists and their fields; read saved views with bounded filtering, sorting, and pagination; and locate the list entries attached to a record. When you explicitly ask for a CRM change, Chat can create text, select, and status fields on a list, add status or select options, update supplied record fields, add an existing record to a list, update an existing list entry, or leave a plaintext comment on a record, list entry, or existing comment thread. Record comments are visible to the workspace, while list-entry comments follow the list's access rules. This lets Chat provision a list pipeline and then assign entries by option title. Writes default to **Ask**, so they pause for in-chat confirmation, and you can set Attio reads and writes to **On**, **Ask**, or **Off** under Settings → Integrations. Chat does not create or delete Attio records, and it does not delete list entries.
