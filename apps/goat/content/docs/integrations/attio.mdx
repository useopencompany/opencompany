...
Goat saves the scopes reported by Attio when the key is validated. For connections saved before scope tracking, Chat revalidates the key's live scopes before deciding whether creation is available; resave the key if that check cannot complete. Chat can search or page through available standard records, query people or companies by interaction recency, and read workspace lists, their entries, and notes. A Chat creation runs only when you explicitly ask for it, supports standard people, companies, enabled deals, and markdown notes, and creates at most one item per capability action. Updates, deletes, and custom objects are not supported.
