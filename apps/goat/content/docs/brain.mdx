...
- **`brain_documents`** — the documents: content, body, timeline JSON, kind, entity type, status, relations, sources, aliases, content hash. Format can be `markdown`, `pdf`, `docx`, `xlsx`, `srt`, or `image`.
