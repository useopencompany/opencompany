...
import { startGoatInfisicalAuth } from "./infisical-auth";

const { currentGoatUser } = vi.hoisted(() => ({
  currentGoatUser: vi.fn(),
}));

vi.mock("@/lib/auth", () => ({ currentGoatUser }));
vi.mock("@opencompany/db/client", () => ({ getDb: vi.fn() }));
vi.mock("next/cache", () => ({ revalidatePath: vi.fn() }));

describe("Goat Infisical auth actions", () => {
  beforeEach(() => {
    currentGoatUser.mockResolvedValue({
      role: "admin",
  ...
  it("accepts a runner login link for the selected EU region", async () => {
    ...
    await expect(
      startGoatInfisicalAuth({ host: "https://eu.infisical.com" }),
