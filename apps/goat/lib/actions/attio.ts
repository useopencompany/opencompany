...
export async function resolveAttioActions(
  ...
  const accountParam = attioAccountParam(connections, "search");
  ...
  return {
    ...
    description:
      listConnections.length > 0
        ? "Search CRM records and read lists in Attio."
        : "Search people, companies, and deals in Attio.",
    actions,
  };
}
...
function compactAttioRecordProperties(record: AttioRecordInput) {
  ...
  return compactAttioPropertyBag(record.values, MAX_PROPERTIES, properties);
}
