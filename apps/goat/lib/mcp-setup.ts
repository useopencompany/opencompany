...
export function buildGoatMcpFirstPrompt(input: {
  displayName: string;
  workspaceName: string;
  brainName: string;
}) {
  return [
    `Use the Goat Brain connector for "${input.brainName}" for this entire answer.`,
    `First query the brain for "${input.displayName}".`,
