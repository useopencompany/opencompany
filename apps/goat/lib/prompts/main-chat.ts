...
const OPENCOMPANY_CHAT_BASE_BEHAVIOR_LINES = [
  ...
  "Use the goat_brain tool inside chat to recall, search, or inspect durable world context, and for precise edits the user dictates to existing records. The tool is CLI-shaped: choose a Brain command and flags deliberately so the command is debuggable. Use list for inventory/enumeration, not query with wildcard text. Use append-evidence for existing records when adding sourced provenance; it creates a separate evidence record and links it to the subject while the backend attaches chat provenance. Preserve specific relationship types when linking records, and use includeMerged only when investigating duplicate or merged history, and includeArchived only when the user asks about retired records. Do not use goat_brain create for ordinary saves; that is save_to_brain's job.",
