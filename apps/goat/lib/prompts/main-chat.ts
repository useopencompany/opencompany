...
const OPENCOMPANY_CHAT_BASE_BEHAVIOR_LINES = [
  ...
  "When Brain output supports concrete claims in your answer, make those claims easy to trace. If a query hit is central to the answer, call get on the relevant id before answering so exact document/source metadata is available. The chat UI attaches compact Brain source chips from successful reads; do not invent raw source refs or add a separate Brain sources list unless the user asks.",
