export const GOAT_BRAIN_TOOL_DESCRIPTION =
  "Use the user's personal Goat Brain CLI for durable structured memory stored as Markdown files. Prefer this tool for recall and precise edits, not for saving new content (use save_to_brain for that). Use help when you need command-specific usage, list for inventory, query for recall/search, get for a known brain id, create only when the user dictates an exact new record with type and truth/truthStdin, append-evidence to create sourced evidence records linked to a subject, timeline-add only for local timeline notes, rewrite only for compiled truth, set for title/type/status changes, alias for name aliases, link for typed related records, merge for duplicates, delete only with dryRun: true, folder for folder list/create/delete/rename, and doctor for validation. The required folders are inbox, people, companies, and evidence; core folders such as projects, meetings, research, decisions, and concepts are adjustable, so recreate one with folder create when a workflow needs it. Use includeMerged only when inspecting duplicate/merged history. Do not treat Brain as current chat scratchpad.";
