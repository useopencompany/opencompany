export const GOAT_BRAIN_TOOL_DESCRIPTION =
  "Use the user's personal Goat Brain CLI for durable structured memory stored as Markdown files. Prefer this tool for recall and precise edits to known existing records, not for saving new content (use save_to_brain for that). Use help when you need command-specific usage, list for inventory, query for recall/search, get for a known brain id, append-evidence for explicit sourced additions to existing records, timeline-add only for local timeline notes, rewrite only for compiled truth on an existing record, set for title/type/status changes, alias for name aliases, link for typed related records, merge for duplicates, move for known existing records, delete only with dryRun: true, and doctor for validation. Do not create new entities or folders from main chat; capture new content with save_to_brain so the background curation agent can file it. Use includeMerged only when inspecting duplicate/merged history. Do not treat Brain as current chat scratchpad.";

export const SAVE_TO_BRAIN_TOOL_DESCRIPTION =
  "Save something the user wants remembered - a reference, idea, note, decision, or pasted content - into their Brain. This captures the content as a draft page in the inbox immediately and queues a background curation agent that files it properly (title, type, folder, links to related pages). Use this whenever the user says things like 'save this', 'remember this', 'note this down', or shares a reference or idea worth keeping. Do not rewrite or summarize the content; capture what the user gave you.";
...
export const GOAT_BRAIN_TOOL_ARGS_DESCRIPTION =
  "CLI-shaped structured invocation. Use { command, flags, stdin? }, where flags are CLI options without leading dashes. Examples: { command: 'help', flags: { topic: 'query' } }, { command: 'append-evidence', flags: { id: 'garry-tan', kind: 'chat', body: 'Met at YC event.', sourceTitle: 'User chat note', json: true } }, { command: 'query', flags: { text: 'hiring', hops: 2, graphDirection: 'both', limit: 5, json: true } }, { command: 'query', flags: { text: 'Sarah Chen', includeMerged: true, json: true } }, { command: 'get', flags: { id: 'garry-tan', json: true } }, or { command: 'delete', flags: { id: 'old-note', dryRun: true, json: true } }. New entities and folders are not supported from main chat; use save_to_brain for new content. Delete is preview-only in chat. Use camelCase or kebab-case flag names.";
