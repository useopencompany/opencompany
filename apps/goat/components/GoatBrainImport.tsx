...
import {
  cancelGoatBrainImportAction,
  confirmGoatBrainImportAction,
  retryGoatBrainImportDiscoveryAction,
...
export function GoatBrainImport({
  brainRef,
  compact = false,
}: {
  brainRef: string;
  ...
  useEffect(() => {
    let canceled = false;
    void getGoatBrainSourcesAction(brainRef).then(async (details) => {
      ...
      if (github?.integrationId && existingRepos.length === 0) {
        const repositories = await listGoatGitHubRepositoriesAction(github.integrationId);
        if (repositories.ok) {
          github.config = {
            ...github.config,
  ...
  const start = () =>
    startTransition(async () => {
      ...
      const result = await startGoatBrainImportDiscoveryAction({
        brainRef,
  ...
  const cancel = () =>
    run &&
    startTransition(async () => {
      const result = await cancelGoatBrainImportAction({
        brainRef,
        importRunId: run.id,
      });
      if (!result.ok) toast.error(result.message);
  ...
  const confirm = () =>
    run &&
    startTransition(async () => {
      const result = await confirmGoatBrainImportAction({
        brainRef,
        importRunId: run.id,
        enabledProviders: Array.from(enabledAtConfirm) as Array<
          "public_web" | "github" | "jamie" | "gmail" | "slack" | "linear"
        >,
      });
      if (!result.ok) toast.error(result.message);
    });
  ...
  if (run.status === "failed")
    return (
      <StatusCard
        ...
        onAction={() =>
          startTransition(async () => {
            const result = await retryGoatBrainImportDiscoveryAction({
              brainRef,
              importRunId: run.id,
            });
            if (!result.ok) toast.error(result.message);
...
function integrationFor(
  details: Awaited<ReturnType<typeof getGoatBrainSourcesAction>>,
  provider: string,
) {
