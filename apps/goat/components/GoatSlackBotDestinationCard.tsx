...
import {
  type GoatSlackBotChannel,
  type GoatSlackBotChannelListResult,
  type GoatSlackBotDestinationView,
  getGoatBrainSlackBotDestinationAction,
  listGoatSlackBotChannelsAction,
  setGoatBrainSlackBotDestinationAction,
...
export function GoatSlackBotDestinationCard({ brainRef }: { brainRef: string }) {
  const [view, setView] = useState<GoatSlackBotDestinationView | null | undefined>(undefined);
  const [isPending, startTransition] = useTransition();

  const reload = async () => {
    const result = await getGoatBrainSlackBotDestinationAction(brainRef);
    setView(result);
  };

  useEffect(() => {
    let cancelled = false;
    void getGoatBrainSlackBotDestinationAction(brainRef).then((result) => {
  ...
  const setEnabled = (next: boolean) => {
    startTransition(async () => {
      const result = await setGoatBrainSlackBotDestinationAction({
        brainRef,
        enabled: next,
        channels,
      });
...
function SlackBotChannelPicker({
  ...
  const [available, setAvailable] = useState<GoatSlackBotChannelListResult | null>(null);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void listGoatSlackBotChannelsAction(brainRef).then((result) => {
  ...
  const toggleChannel = (channel: GoatSlackBotChannel) => {
    setDirty(true);
