...
export function FathomIntegrationSetup({
  ...
  function saveApiKey() {
    ...
    startTransition(async () => {
      const result = await saveFathomApiKeyAction(apiKey);
      if (!result.ok) {
    ...
    <div className="flex flex-col gap-8">
      ...
      <section className="flex flex-col gap-3">
          ...
          <div className="mx-2 flex items-start gap-2 rounded-md border border-border bg-surface-muted px-2.5 py-2">
            ...
            <div className="min-w-0">
              ...
              <span className="block text-[12px] leading-4 text-ink-subtle">
                Paste a new Fathom API key below to replace it.
              </span>
            </div>
          </div>
        ) : null}
        <label className="flex flex-col gap-1 px-2">
          ...
          <input
            ...
            autoComplete="off"
            spellCheck={false}
            placeholder={state.connected ? "Paste a new Fathom API key" : "Fathom API key"}
            disabled={isPending}
        ...
        <div className="flex flex-col items-start gap-3 px-2 sm:flex-row sm:items-center sm:justify-between">
          <span className="min-w-0 text-[12px] leading-4 text-ink-subtle">
            {state.connected
              ...
              : "Create the key in Fathom, then paste it here."}
          </span>
      ...
      <section className="flex flex-col gap-2">
        ...
        <ol className="list-decimal space-y-2 pl-6 text-[13px] leading-5 text-ink-subtle">
          <li>In Fathom, open Settings and go to the API Access section.</li>
          <li>
            Generate a new API key. It can read meetings you recorded or that are shared with your
            team.
          </li>
...
function setupStatus(state: GoatFathomProviderState) {
  ...
  if (state.status === "needs_reauth" || state.status === "sync_failed") {
    return {
      ...
      detail: state.statusReason ?? "Fathom rejected the saved key. Paste a new one below.",
      badge: "Reconnect",
    };
  }
  return {
    label: "Not connected",
    detail: "Paste a Fathom API key to ingest your meeting recordings.",
    badge: "Setup",
  };
}
