...
const base: GoatBillingPanelData = {
  ...
  seatCount: 3,
  seatPriceEurCents: 1_500,
  monthlySubtotalEurCents: 4_500,
...
describe("GoatBillingPanel", () => {
  it("shows the Free allowance and admin upgrade action", () => {
    ...
    expect(screen.getByText(/200 ingestions per calendar month/i)).toBeInTheDocument();
    expect(screen.getByText("42 ingestions")).toBeInTheDocument();
    expect(screen.getByText(/42 of 200 monthly allowance/i)).toBeInTheDocument();
    expect(screen.getByText("Allowance resets").parentElement?.parentElement).toHaveTextContent(
      "Aug 1, 2026",
    );
  });

  it("shows Pro seat pricing, cancellation, and payment state", () => {
    render(
      <GoatBillingPanel
        data={{
          ...base,
    ...
    expect(screen.getByText(/€15\.00 × 3 active seats/i)).toBeInTheDocument();
