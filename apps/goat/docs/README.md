...
search, live Google Doc reads, and exact text replacement in Google Docs. Linear exposes a curated
catalog for reading issues and workspace context, creating and updating issues, and adding comments.
Attio exposes bounded fuzzy search across standard people,
companies, and deals; list, field, and membership discovery; and bounded list reads with saved-view
filters, explicit filters, sorting, and pagination. Explicitly requested Google Doc edits, Attio
record and list-entry updates, Linear writes, and Google Calendar event creation require confirmation
by default and can be configured under Integrations. An explicit account or workspace is required
when several are connected. Stripe exposes read-only workspace metrics for balance activity by
period, current balances, subscription health with estimated MRR, and open receivables. Stripe uses an encrypted
