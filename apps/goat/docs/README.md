...
the client can confirm ownership while persisted chat sessions and messages arrive through TanStack
DB collections backed by Electric shapes. Other persisted Goat app state such as tasks, task run
events, integrations, and Brain documents uses the same live-data path.
