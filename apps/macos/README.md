# opencompany Quick for macOS

opencompany Quick is a native macOS 14+ menu-bar utility. Press `Option-Command-Space` from any app to open a compact opencompany composer. Submitting creates a normal opencompany chat session, hides the panel immediately, and drains the response stream in the background.
...
WorkOS documents the native loopback flow in [Connect with OAuth](https://workos.com/docs/authkit/connect/oauth). The wildcard port is intentional: opencompany Quick binds only to `127.0.0.1` on a new ephemeral port for each sign-in.

Configure the opencompany app server (`apps/app`, deployed at `my.opencompany.chat`) with the same environment and application:

```dotenv
AUTHKIT_DOMAIN="https://your-environment.authkit.app"
MACOS_OAUTH_AUDIENCE="client_..."
MACOS_OAUTH_CLIENT_ID="client_..."
NEXT_PUBLIC_APP_URL="https://my.opencompany.chat"
```
...
`sub` and `org_id` to an onboarded opencompany user and workspace membership.
...
```sh
cp apps/macos/Config/Local.xcconfig.example apps/macos/Config/Local.xcconfig
```

`Local.xcconfig` is gitignored. Debug builds use `http://127.0.0.1:3002` for the opencompany app by default. Override `GOAT_API_BASE_URL` for another deployment. Release values should be passed by CI or a local uncommitted xcconfig; no credentials belong in the project.

Start the opencompany development stack with `bun run dev`, open `apps/macos/GoatQuick.xcodeproj`, select the `GoatQuick` scheme, and run it. It appears only in the menu bar. (The Xcode project, scheme, and build settings keep their legacy `GoatQuick`/`GOAT_` names.)
...
- Every submission gets new `goat_chat_<uuid>` and `goat_chat_msg_<uuid>` identifiers (the `goat_chat_` row-id prefix is a frozen storage contract).
...
- If a stream disconnects after a successful HTTP response, opencompany Quick preserves the session ID and offers a manual **Open in opencompany** link.
