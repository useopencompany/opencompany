...
describe("resolveApiPort", () => {
  ...
  it("uses PORT for hosted API origins", () => {
    expect(
      resolveApiPort({ OPENCOMPANY_API_ORIGIN: "https://api.opencompany.chat", PORT: "10000" }),
    ).toBe(10000);
  });

  it("uses an isolated localhost API origin instead of the runner's shared PORT", () => {
    expect(
      resolveApiPort({ OPENCOMPANY_API_ORIGIN: "http://localhost:55014", PORT: "55011" }),
    ).toBe(55014);
  });

  it("rejects an invalid hosted listener port", () => {
    expect(() =>
      resolveApiPort({ OPENCOMPANY_API_ORIGIN: "https://api.opencompany.chat", PORT: "invalid" }),
