...
export function createRunnerClient(
  ...
  return {
    async postJson<TResponse>(
      ...
      // Resolved per call (like the retired web helpers) so the service boots
      // even when the runner env lands after the API process starts.
      const baseUrl = (
        input.url?.trim() ||
      ...
      if (!response.ok) {
        ...
        const reason =
          typeof payload?.error === "string" ? payload.error : "Runner request failed.";
