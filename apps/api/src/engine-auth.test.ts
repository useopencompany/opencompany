...
describe("engine auth service", () => {
  ...
  it("refreshes installed Infisical plugin discovery after a completed connection", async () => {
    const completedFlow = { ...infisicalFlow, status: "completed" as const, loginUrl: null };
  ...
  it("does not fail a completed Infisical connection when plugin refresh fails", async () => {
    const completedFlow = { ...infisicalFlow, status: "completed" as const, loginUrl: null };
    const { runner } = fakeRunner([{ ok: true, flow: completedFlow }]);
