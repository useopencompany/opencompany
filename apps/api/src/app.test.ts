...
describe("canonical Hono API", () => {
  ...
  it("binds every provider ingress route to its service outside the /v1 middleware", async () => {
    ...
    const app = testApp(fakeRepository(), {
      ...
      microsoftIngress: {
        start: recordMcp("microsoft.start", calls),
        callback: recordMcp("microsoft.callback", calls),
      },
      outlookMcp: {
        handle: record("outlook.mcp", calls),
        downloadAttachment: record("outlook.download", calls),
      },
      outlookCalendarMcp: { handle: record("outlook-calendar.mcp", calls) },
      googleIngress: {
    ...
    const routes: Array<[string, string, string]> = [
      ...
      ["GET", "/integrations/outlook/start", "microsoft.start.outlook"],
      ["GET", "/integrations/outlook/callback", "microsoft.callback.outlook"],
      ["GET", "/integrations/outlook-calendar/start", "microsoft.start.outlook-calendar"],
      ["GET", "/integrations/outlook-calendar/callback", "microsoft.callback.outlook-calendar"],
      ["POST", "/mcp/plugins/outlook", "outlook.mcp"],
      ["POST", "/mcp/plugins/outlook-calendar", "outlook-calendar.mcp"],
      ["GET", "/mcp/plugins/outlook/attachments/download", "outlook.download"],
