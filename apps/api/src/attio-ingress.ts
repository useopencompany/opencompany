...
import { verifyAttioWebhookSignature } from "@opencompany/agent/integrations/attio-signature";
import {
...
import type { AttioEventAction, AttioObjectType } from "@opencompany/db/product-schema";
import { createLogger } from "@opencompany/observability";

const logger = createLogger({ service: "opencompany-api", runtime: "attio-ingress" });

type DbLike = any;

// Provider ingress composition for the Attio Brain-ingestion connection: the
// webhook that buffers CRM record and note events for the runner's flush
// worker. Connect is API-key based (a web Server Action), so there is no OAuth
// surface here.
export type AttioIngressService = {
  webhook(request: Request): Promise<Response>;
};

export function createAttioIngress(input: { db: DbLike }): AttioIngressService {
  return {
    webhook: (request) => handleWebhook(input.db, request),
  };
}
...
async function handleWebhook(db: DbLike, request: Request): Promise<Response> {
  ...
  try {
    const match = await findIntegrationForWebhook(db, workspaceId, webhookId);
    if (!match) {
      return Response.json({ error: "Unknown Attio webhook." }, { status: 401 });
    }
    ...
    if (!verified) {
      return Response.json({ error: "Invalid Attio signature." }, { status: 401 });
    }
    return Response.json(
      await handleAttioEvents(db, { events, workspaceId, idempotencyKey, match }),
    );
  } catch (error) {
    logger.error("Failed to process Attio events", {
      event: "goat.attio_events_failed",
      event_count: events.length,
      error_message: error instanceof Error ? error.message : String(error),
    });
    return Response.json({ error: "Unable to buffer Attio events." }, { status: 503 });
  }
}
...
async function findIntegrationForWebhook(
  ...
  const integrations = await listAttioIntegrationsForWorkspace(workspaceId, db);
...
async function handleAttioEvents(
  db: DbLike,
  input: {
    events: AttioWebhookEvent[];
    workspaceId: string;
  ...
  const routes = await listEnabledAttioBrainSourceRoutes([match.integrationId], db);
  ...
  const buffered = await insertAttioObjectEvents(inserts, db);
