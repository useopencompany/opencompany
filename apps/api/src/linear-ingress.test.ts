...
import { createGoatLinearIngestState } from "@opencompany/goat-agent/integrations/linear-ingest";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import { ApiError } from "./errors";
import { createLinearIngress } from "./linear-ingress";

vi.mock("@opencompany/db/goat-linear", async (importOriginal) => {
  const original = await importOriginal<typeof import("@opencompany/db/goat-linear")>();
  return {
    ...original,
    insertGoatLinearIssueEvents: vi.fn(),
    listEnabledGoatLinearBrainSourceRoutes: vi.fn(),
    listGoatLinearIntegrationsForOrganization: vi.fn(),
  };
});
...
describe("Linear ingress", () => {
  beforeEach(() => {
    ...
    vi.mocked(listGoatLinearIntegrationsForOrganization).mockResolvedValue([
      { id: "gint_1", userWorkosId: "user_1", status: "connected" },
    ] as never);
    vi.mocked(listEnabledGoatLinearBrainSourceRoutes).mockResolvedValue([
      {
    ...
    vi.mocked(insertGoatLinearIssueEvents).mockResolvedValue(1);
  });
  ...
  describe("webhook", () => {
    it("rejects a bad signature", async () => {
      ...
      expect(insertGoatLinearIssueEvents).not.toHaveBeenCalled();
    });
    ...
    it("buffers a selected-team issue event through the injected db", async () => {
      ...
      expect(insertGoatLinearIssueEvents).toHaveBeenCalledWith(
        [
    ...
    it("acks with 200 when processing fails after verification", async () => {
      vi.mocked(listGoatLinearIntegrationsForOrganization).mockRejectedValue(new Error("db down"));
      const response = await ingress().webhook(signedRequest(issueEnvelope()));
      expect(response.status).toBe(200);
