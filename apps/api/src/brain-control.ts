...
import type { BrainIntelligence, BrainVisibility } from "@opencompany/db/product-schema";
import {
  createBrain,
  getBrainAccess,
  listBrainMemberIds,
...
export type BrainControlService = {
  ...
  createBrain(
    actor: Actor,
    input: { name: string; visibility: BrainVisibility; description?: string },
  ...
  setAccess(
    actor: Actor,
    brainId: string,
    input: { visibility: BrainVisibility; memberIds: string[] },
  ): Promise<void>;
  ...
  getIntelligence(actor: Actor, brainId: string): Promise<{ intelligence: BrainIntelligence }>;
  setIntelligence(actor: Actor, brainId: string, intelligence: BrainIntelligence): Promise<void>;
};
...
export function createBrainControlService(input: { db: DbLike }): BrainControlService {
  ...
  return {
    ...
    async setEnrichment(actor, brainId, enabled) {
      ...
      await requireWorkspaceBrain(db, actor, brainId);
      await updateBrainEnrichmentEnabled({ brainRef: brainId, enabled }, { db });
    },
    ...
    async setIntelligence(actor, brainId, intelligence) {
      ...
      await requireWorkspaceBrain(db, actor, brainId);
      await updateBrainIntelligence({ brainRef: brainId, intelligence }, { db });
    },
  };
}
