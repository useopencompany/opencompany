...
vi.mock("@opencompany/db/slack", async (importOriginal) => {
  ...
  return {
    slackSelectedConversationIds: original.slackSelectedConversationIds,
...
function signedRequest(
  ...
  return new Request("https://app.example.com/api/webhooks/slack/events", {
    method: "POST",
    headers: {
...
describe("POST /api/webhooks/slack/events", () => {
  ...
  it("marks only the revoked user's integration on tokens_revoked", async () => {
    ...
    vi.mocked(loadIntegrationCredential).mockImplementation(async (input) => ({
