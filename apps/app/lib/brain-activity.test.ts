...
function job(overrides: Partial<BrainIngestJobRow> = {}): BrainIngestJobRow {
  return {
    id: "gbjob_1",
...
function item(overrides: Partial<BrainSourceItemRow> = {}): BrainSourceItemRow {
  return {
    id: "gbsrc_1",
    user_workos_id: "user_1",
    source_provider: "goat-chat",
...
describe("buildBrainActivityEvents", () => {
  ...
  it("shows queued work held by a quota reservation as paused by plan", () => {
    const events = buildBrainActivityEvents([job({ plan_paused: true })], [item()]);
