...
// httpOnly cookie instead — mirrors setOAuthStateCookie in auth-methods.ts.
const PKCE_COOKIE = "goat_x_pkce_verifier";

export async function setXAccountPkceCookie(codeVerifier: string) {
...
export async function consumeXAccountPkceCookie(): Promise<string | null> {
  const cookieStore = await cookies();
