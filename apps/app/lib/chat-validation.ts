...
export const CHAT_PROMPT_MAX_LENGTH = 10_000;
...
export const CHAT_OUT_OF_CREDITS_MESSAGE =
...
export function validateChatInput(input: {
  ...
  if (prompt.length > CHAT_PROMPT_MAX_LENGTH) {
    return {
      ok: false,
      error: `Messages can be at most ${CHAT_PROMPT_MAX_LENGTH.toLocaleString()} characters.`,
    };
  }
