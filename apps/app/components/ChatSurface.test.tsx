...
vi.mock("@/lib/chat-actions", () => ({
  closeChatSessionAction: vi.fn(async () => ({ ok: true, error: null })),
  ...
  markChatSeenAction: vi.fn(async () => ({ ok: true, error: null })),
...
vi.mock("@/lib/chat-attachment-upload", () => ({
  uploadChatAttachmentBlob: attachmentUploadMock.upload,
}));

vi.mock("@/lib/tasks", () => ({
  archiveTaskAction: vi.fn(async () => ({ ok: true, error: null })),
  cancelTaskAction: vi.fn(async () => ({ ok: true, error: null })),
  continueTaskAction: vi.fn(async (_taskId: string, _prompt: string, messageId: string) => ({
    ok: true,
    error: null,
    messageId,
  })),
...
vi.mock("@/lib/task-schedules", () => ({
  deleteTaskScheduleAction: vi.fn(async () => ({ ok: true })),
  runTaskScheduleNowAction: vi.fn(async () => ({
    ok: true,
    task: { id: "goat_task_1", displayId: "TASK-1" },
  })),
  setTaskScheduleEnabledAction: vi.fn(async () => ({ ok: true })),
  updateTaskScheduleAction: vi.fn(async () => ({ ok: true })),
}));
...
describe("ChatSurface chat streaming UI", () => {
  beforeEach(() => {
    ...
    vi.mocked(closeChatSessionAction).mockClear();
    vi.mocked(markChatSeenAction).mockClear();
    vi.mocked(cancelTaskAction).mockClear();
    vi.mocked(continueTaskAction).mockClear();
  ...
  it("marks an already-open chat seen again after a live assistant message finishes", async () => {
    ...
    await waitFor(() => expect(markChatSeenAction).toHaveBeenCalledWith(initialChat.id));
    vi.mocked(markChatSeenAction).mockClear();
  ...
  it("continues a session-backed workflow task through the same chat composer", async () => {
    ...
    await waitFor(() =>
      expect(continueTaskAction).toHaveBeenCalledWith(
        "goat_task_1",
  ...
  it("uses the chat stop control for an active workflow task", async () => {
    ...
    expect(cancelTaskAction).toHaveBeenCalledWith("goat_task_1");
    expect(chatMock.stop).not.toHaveBeenCalled();
  ...
  it("restores the task stop control when cancellation fails", async () => {
    const user = userEvent.setup();
    vi.mocked(cancelTaskAction).mockResolvedValueOnce({
      ok: false,
  ...
  it("remembers the last main chat model when returning Home and remounting", async () => {
    ...
    const { unmount } = render(<ChatSurface {...props} />);
  ...
  it("remembers the last main chat engine across a Home reset and remounting", async () => {
    ...
    const { unmount } = render(<ChatSurface {...sharedProps} initialChat={null} />);
    ...
    unmount();
    render(<ChatSurface {...sharedProps} initialChat={null} />);
  ...
  it("archives a chat from the home list", async () => {
    ...
    expect(closeChatSessionAction).toHaveBeenCalledWith("chat_1");
  ...
  it("keeps chat closed while server props refresh after Escape", async () => {
    ...
    expect(closeChatSessionAction).not.toHaveBeenCalled();
  });
