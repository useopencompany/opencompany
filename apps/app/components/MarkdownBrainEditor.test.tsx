...
describe("MarkdownBrainEditor", () => {
  ...
  it("does not fire onChange when props change identity without an edit", async () => {
    ...
    const { rerender } = render(<MarkdownBrainEditor {...props} onNavigateInternal={() => true} />);
    ...
    rerender(<MarkdownBrainEditor {...props} onNavigateInternal={() => true} />);
    rerender(<MarkdownBrainEditor {...props} onNavigateInternal={() => true} />);
  ...
  it("uses the latest internal navigation handler after a prop update", async () => {
    ...
    const { rerender } = render(
      <MarkdownBrainEditor {...props} onNavigateInternal={firstHandler} />,
    );

    const link = await screen.findByRole("link", { name: "Acme" });
    rerender(<MarkdownBrainEditor {...props} onNavigateInternal={secondHandler} />);
    expect(fireEvent.click(link)).toBe(false);
