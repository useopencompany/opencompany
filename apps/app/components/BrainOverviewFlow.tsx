...
export function BrainOverviewFlow({
  brain,
  workspace,
  onManageAccess,
}: {
  ...
  const [configuring, setConfiguring] = useState<BrainSourceProviderDef | null>(null);

  const reloadSources = useCallback(async () => {
    const next = await getBrainSourcesAction(brain.id);
    setDetails(next);
  }, [brain.id]);

  useEffect(() => {
    let cancelled = false;
    void getBrainSourcesAction(brain.id).then((next) => {
      if (!cancelled) setDetails(next);
    });
    void getBrainAccessDetailsAction(brain.id).then((next) => {
...
function FlowDiagram({
  ...
  people: WorkspaceMemberView[] | null;
  onConfigure: (provider: BrainSourceProviderDef) => void;
  onManageAccess: () => void;
}) {
