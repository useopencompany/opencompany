...
import { AppSidebarFeedback } from "@/components/AppSidebarFeedback";
...
export function AppSidebar({
  ...
  const { featureFlags, mcpSetup, user } = useAppData();
...
function AppSidebarRecentChats() {
  ...
  const archiveChat = (chatId: string, chatTitle: string, href: string) => {
    ...
    startTransition(async () => {
      const result = await closeChatSessionAction(chatId);
  ...
  const togglePin = (chatId: string, chatTitle: string, currentlyPinned: boolean) => {
    ...
    startTransition(async () => {
      try {
        const result = await setChatPinnedAction(chatId, desiredPinned);
        if (result.ok) return;
