...
import { createWorkflowAction } from "@/lib/workflow-actions";
  ...
  useEffect(() => {
    ...
    void fetch(`/api/tasks/${encodeURIComponent(taskId)}/run`, {
      ...
      .then(async (response) => {
        ...
        return (await response.json()) as HarnessRunViewModel;
      })
      .then((run) => {
...
export function SkillEditorRoute({
  ...
  const archive = () => {
    ...
    startArchiving(async () => {
      const result = await archiveSkillAction({ slug: skill.id });
      if (result.ok) {
