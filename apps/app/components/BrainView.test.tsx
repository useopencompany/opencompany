...
vi.mock("@/components/BrainSwitcher", () => ({
  BrainAccessDialog: () => null,
}));

vi.mock("@/components/BrainActivity", () => ({
  BrainActivity: ({ brainRef }: { brainRef: string }) => (
    <span data-testid="brain-activity">{brainRef}</span>
  ),
  BrainRecentActivity: () => null,
}));

vi.mock("@/components/BrainOverview", () => ({
  BrainOverview: ({ brainName }: { brainName: string }) => (
    <div data-testid="brain-overview">{brainName}</div>
  ),
}));

vi.mock("@/components/BrainImport", () => ({
  BrainImport: ({ brainRef }: { brainRef: string }) => (
...
vi.mock("@/lib/brain-actions", () => ({
  createBrainDocumentAction: vi.fn(),
  createBrainFolderAction: vi.fn(),
  createBrainSkillAction: vi.fn(),
  deleteBrainDocumentAction: vi.fn(),
  deleteBrainFolderAction: vi.fn(),
  moveBrainDocumentAction: vi.fn(),
  renameBrainDocumentAction: vi.fn(),
  renameBrainFolderAction: vi.fn(),
  updateBrainDocumentAction: vi.fn(),
  updateBrainSkillAction: vi.fn(),
}));
...
describe("BrainView", () => {
  ...
  it("renders workspace-member brain access as browse-only", async () => {
    ...
    expect(updateBrainDocumentAction).not.toHaveBeenCalled();
    expect(renameBrainDocumentAction).not.toHaveBeenCalled();
  });

  it("creates a nested folder from the sidebar context menu", async () => {
    const user = userEvent.setup();
    vi.mocked(createBrainFolderAction).mockResolvedValueOnce({
      ok: true,
    ...
    await waitFor(() => {
      expect(createBrainFolderAction).toHaveBeenCalledWith({
        brainRef: "goat_brain_1",
  ...
  it("creates a Markdown file in the context-clicked folder", async () => {
    const user = userEvent.setup();
    vi.mocked(createBrainDocumentAction).mockResolvedValueOnce({
      ok: true,
    ...
    await waitFor(() => {
      expect(createBrainDocumentAction).toHaveBeenCalledWith({
        brainRef: "goat_brain_1",
        folderPath: "projects",
        fileName: "Roadmap.md",
      });
  ...
  it("creates a formal skill with only a name from the skills folder", async () => {
    const user = userEvent.setup();
    vi.mocked(createBrainSkillAction).mockResolvedValueOnce({
      ok: true,
    ...
    await waitFor(() => {
      expect(createBrainSkillAction).toHaveBeenCalledWith({
        brainRef: "goat_brain_1",
        folderPath: "skills",
        name: "Coding work",
        description: "",
  ...
  it("edits skill description and instructions without timeline controls", async () => {
    const user = userEvent.setup();
    vi.mocked(updateBrainSkillAction).mockResolvedValueOnce({
      ok: true,
    ...
    await waitFor(
      () => {
        expect(updateBrainSkillAction).toHaveBeenCalledWith({
          brainRef: "goat_brain_1",
  ...
  it("renames only adjustable folders", async () => {
    const user = userEvent.setup();
    vi.mocked(renameBrainFolderAction).mockResolvedValueOnce({
      ok: true,
      path: "initiatives",
    });
    ...
    await waitFor(() => {
      expect(renameBrainFolderAction).toHaveBeenCalledWith({
        brainRef: "goat_brain_1",
        fromPath: "projects",
        toPath: "initiatives",
      });
  ...
  it("deletes only adjustable folders after confirmation", async () => {
    ...
    vi.mocked(deleteBrainFolderAction).mockResolvedValueOnce({ ok: true });
    ...
    await waitFor(() => {
      expect(deleteBrainFolderAction).toHaveBeenCalledWith({
        brainRef: "goat_brain_1",
        folderPath: "projects",
      });
    });
  ...
  it("renames a document from the page title", async () => {
    const user = userEvent.setup();
    vi.mocked(renameBrainDocumentAction).mockResolvedValueOnce({
      ok: true,
    ...
    await waitFor(() => {
      expect(renameBrainDocumentAction).toHaveBeenCalledWith({
        brainRef: "goat_brain_1",
        documentId: "doc_ada",
        title: "Ada King",
      });
  ...
  it("autosaves the body with the selected content hash after typing pauses", async () => {
    const user = userEvent.setup();
    vi.mocked(updateBrainDocumentAction).mockResolvedValueOnce({
      ok: true,
      path: "people/ada-lovelace.md",
    ...
    await waitFor(
      () => {
        expect(updateBrainDocumentAction).toHaveBeenCalledWith({
          brainRef: "goat_brain_1",
          documentId: "doc_ada",
          body: "Updated truth.",
    ...
    expect(updateBrainDocumentAction).toHaveBeenCalledTimes(1);
  });
...
const orderedFolders: BrainFolderView[] = [
...
const documentWithTimeline: BrainDocumentView = {
  id: "doc_ada",
...
const documentLinkingToAda: BrainDocumentView = {
  id: "doc_roadmap",
  brainId: "roadmap",
  folderPath: "projects",
  path: "projects/roadmap.md",
...
const codingWorkSkill: BrainDocumentView = {
  ...documentLinkingToAda,
  id: "doc_coding_work",
  brainId: "coding-work",
  folderPath: "skills",
...
const inboxCaptureDocument: BrainDocumentView = {
...
const evidenceDocument: BrainDocumentView = {
...
const documentWithGitHubSource: BrainDocumentView = {
...
function folder(folderPath: string, source: BrainFolderView["source"]): BrainFolderView {
  return {
