...
import {
  ...
  brainRootFolderGroup,
  compareBrainFolderPaths,
} from "@opencompany/brain/folders";
import {
  evidenceLinkTargets,
...
import { useNavInset } from "@/components/NavInset";
import { useHydrated } from "@/components/useHydrated";
import type { BrainDocumentView, BrainFolderView } from "@/lib/brain";
import {
  createBrainDocumentAction,
  createBrainFolderAction,
  createBrainSkillAction,
  createBrainWorkflowAction,
  deleteBrainDocumentAction,
  deleteBrainFolderAction,
  renameBrainDocumentAction,
  renameBrainFolderAction,
  replaceBrainAssetAction,
  updateBrainDocumentAction,
  updateBrainSkillAction,
  updateBrainWorkflowAction,
  uploadBrainAssetAction,
} from "@/lib/brain-actions";
import { type BrainDraftIngestState, buildBrainDraftIngestStates } from "@/lib/brain-activity";
import {
  BRAIN_ASSET_ACCEPT,
...
import {
  ...
  type BrainEdgeRow,
  type BrainFolderRow,
  type BrainIngestJobRow,
  type BrainSourceItemRow,
  type BrainTimelineEntryRow,
  createCollections,
} from "@/lib/task-collections";

type Props = {
  brainRef: string | null;
  brain: BrainSummaryView | null;
  ...
  overviewStats?: BrainOverviewStats | null;
...
type BrainTreeNode = {
  ...
  folder?: BrainFolderView;
  document?: BrainDocumentView;
};
...
const EMPTY_DRAFT_INGEST_STATES: ReadonlyMap<string, BrainDraftIngestState> = new Map();
const EMPTY_OVERVIEW_STATS: BrainOverviewStats = {
...
function LiveBrainView({
  ...
  const documents = useMemo(() => {
    ...
    const timelinesByDocument = groupTimelineRows((timelineRows ?? []) as BrainTimelineEntryRow[]);
    return ((fileRows ?? []) as BrainDocumentRow[])
  ...
  const folders = useMemo(() => {
    ...
    return deriveFolderViews(documents, (folderRows ?? []) as BrainFolderRow[]).filter(
  ...
  const draftIngestStatesByBrainId = useMemo(
    () =>
      buildBrainDraftIngestStates(
        (ingestJobRows ?? []) as BrainIngestJobRow[],
        (captureSourceItemRows ?? []) as BrainSourceItemRow[],
      ),
  ...
  return (
    <BrainEditor
      ...
      folders={folders}
      documents={documents}
      edgeRows={(edgeRows ?? []) as BrainEdgeRow[]}
...
function BrainEditor({
  ...
  edgeRows?: BrainEdgeRow[];
  draftIngestStatesByBrainId?: ReadonlyMap<string, BrainDraftIngestState>;
  ...
  const [optimisticDocumentState, setOptimisticDocumentState] = useState<{
    syncedDocuments: BrainDocumentView[];
    createdDocuments: BrainDocumentView[];
  ...
  const isSelectedWorkflow = Boolean(
    selectedDocument && isBrainWorkflowFolder(selectedDocument.folderPath),
  );
  const dirty = Boolean(
    selectedDocument &&
  ...
  const selectDocument = (document: BrainDocumentView) => {
  ...
  const saveDocument = () => {
    ...
    startDocTransition(async () => {
      const result = isBrainSkillFolder(selectedDocument.folderPath)
        ? await updateBrainSkillAction({
            brainRef,
            documentId,
        ...
        : isBrainWorkflowFolder(selectedDocument.folderPath)
          ? await updateBrainWorkflowAction({
              brainRef,
              documentId,
              name: selectedDocument.title,
              description: editorDescription,
  ...
  const renameDocument = (title: string) => {
    ...
    startDocTransition(async () => {
      const result = await renameBrainDocumentAction({
        brainRef,
  ...
  const deleteDocument = () => {
    ...
    startDocTransition(async () => {
      ...
      const result = await deleteBrainDocumentAction({
        brainRef,
  ...
  const submitSkillDialog = (name: string, description: string) => {
    const folderPath = fileDialogFolder;
    if (!brainRef || !folderPath || !canEditBrain) return;
    const creatingWorkflow = isBrainWorkflowFolder(folderPath);
    startCreateTransition(async () => {
      const result = creatingWorkflow
        ? await createBrainWorkflowAction({
            brainRef,
            folderPath,
            name,
            description,
          })
        : await createBrainSkillAction({
            brainRef,
            folderPath,
            name,
            description,
  ...
  const submitFolderDialog = (folderPath: string) => {
    ...
    startFolderTransition(async () => {
      const result =
        dialog.kind === "create"
          ? await createBrainFolderAction({ brainRef, folderPath: trimmed })
          : await renameBrainFolderAction({
              brainRef,
  ...
  const deleteFolder = (folderPath: string) => {
    ...
    startFolderTransition(async () => {
      const result = await deleteBrainFolderAction({
        brainRef,
        folderPath,
      });
      if (!result.ok) {
  ...
  const selectUploadedDocument = (document: BrainDocumentView) => {
    setOverviewSelected(false);
    setSelectedFolder(document.folderPath);
    setSelectedDocumentId(document.id);
  ...
  const uploadAssetFile = async (file: File | undefined) => {
    ...
    if (isSkillLikeBrainFolder(activeFolder)) {
      toast.error(
        isBrainWorkflowFolder(activeFolder)
    ...
    try {
      ...
      const result = await uploadBrainAssetAction({
        brainRef,
  ...
  const replaceAssetFile = async (file: File | undefined) => {
    ...
    try {
      const uploaded = await uploadBrainAssetBlob(brainRef, file);
      const result = await replaceBrainAssetAction({
        brainRef,
        documentId: selectedDocument.id,
    ...
    >
      <aside className="flex w-60 shrink-0 flex-col border-r border-border-subtle bg-surface-muted">
        ...
        >
          ...
          <div className="flex shrink-0 items-center">
            {brainRef ? <BrainActivity brainRef={brainRef} /> : null}
      ...
      {fileDialogFolder && canEditBrain ? (
        isSkillLikeBrainFolder(fileDialogFolder) ? (
          <SkillDialog
            kind={isBrainWorkflowFolder(fileDialogFolder) ? "workflow" : "skill"}
...
function TreeItem({
  ...
  draftIngestStatesByBrainId: ReadonlyMap<string, BrainDraftIngestState>;
  onSelect: (document: BrainDocumentView) => void;
...
function TreeIngestStatusSlot({ state }: { state: BrainDraftIngestState | null }) {
...
function BrainDocumentPanel({
  ...
  selectedDocument: BrainDocumentView | null;
  documents: BrainDocumentView[];
  ...
  const isWorkflow = isBrainWorkflowFolder(selectedDocument.folderPath);
...
function documentShowsAttribution(document: BrainDocumentView) {
  return (
...
function MetadataSourcesRow({ sources }: { sources: BrainDocumentView["sources"] }) {
  return (
    <>
...
function SidebarTimelineSection({ document }: { document: BrainDocumentView }) {
...
function resolveGraphLinks(
  ...
  documentsByBrainId: Map<string, BrainDocumentView>,
  direction: "out" | "in",
...
function BrainDocumentTimeline({ document }: { document: BrainDocumentView }) {
              ...
              >
                <div className="min-w-0">
                  ...
                  <code className="mt-1 block truncate rounded-sm bg-surface px-1 py-0.5 text-[11px] text-ink-muted">
                    {formatBrainEvidenceLink(entry.evidenceId)}
                  </code>
                </div>
...
function groupRootNodes(nodes: BrainTreeNode[]) {
  ...
  for (const node of visibleNodes) {
    ...
    groups[brainRootFolderGroup(root)]?.push(node);
  }
...
function buildBrainTree(folders: BrainFolderView[], documents: BrainDocumentView[], query: string) {
...
function insertFolder(root: BrainTreeNode, path: string, folder?: BrainFolderView) {
...
function insertDocument(root: BrainTreeNode, document: BrainDocumentView) {
...
function sortTreeNodes(nodes: BrainTreeNode[]) {
  nodes.sort((a, b) => {
    ...
    if (a.type === "folder" && b.type === "folder") {
      return compareBrainFolderPaths(a.path, b.path);
    }
...
function draftIngestStateLabel(state: BrainDraftIngestState) {
...
function brainDocumentUrl(document: BrainDocumentView, routeBrainId?: string | null) {
...
function brainDocumentTreePath(document: BrainDocumentView) {
...
function documentInlineLinkText(document: BrainDocumentView) {
  return [documentEditorBody(document), ...document.timeline.map((entry) => entry.body)].join(
    "\n\n",
  );
}

function documentEditorBody(document: BrainDocumentView | null | undefined) {
  if (!document) return "";
...
function isSkillLikeBrainFolder(path: string) {
  return isBrainSkillFolder(path) || isBrainWorkflowFolder(path);
}
...
function documentWorkflowModel(document: BrainDocumentView | null | undefined) {
  if (!document || !isBrainWorkflowFolder(document.folderPath)) return "";
  return parseBrainDocument(document.content).frontmatter.model?.trim() ?? "";
}

function brainFileKindForFolder(path: string): "skill" | "workflow" | "file" {
  if (isBrainSkillFolder(path)) return "skill";
  if (isBrainWorkflowFolder(path)) return "workflow";
  return "file";
}
...
function documentViewFromRow(
  ...
  timelineRows?: BrainTimelineEntryRow[],
): BrainDocumentView {
  const path = `${row.folder_path}/${row.brain_id}.md`;
  const title = row.title ?? row.brain_id;
  let description: string | undefined;
  try {
    description = parseBrainDocument(row.content).frontmatter.description;
  } catch {}
  return {
    id: row.id,
...
function groupTimelineRows(rows: BrainTimelineEntryRow[]) {
  const byDocument = new Map<string, BrainTimelineEntryRow[]>();
...
function timelineRowsFromRows(rows: BrainTimelineEntryRow[]): BrainDocumentView["timeline"] {
  return rows.map((row) => ({
...
function sourceLine(row: BrainTimelineEntryRow) {
...
function normalizeTimeline(value: BrainDocumentRow["timeline"]): BrainDocumentView["timeline"] {
  return Array.isArray(value)
    ? value.flatMap((entry): BrainDocumentView["timeline"] => {
...
function normalizeFormat(value: string): BrainDocumentView["format"] {
  if (
    value === "pdf" ||
    value === "docx" ||
    value === "xlsx" ||
...
function normalizeDocumentKind(value: string): BrainDocumentView["kind"] {
  return value === "evidence" ? "evidence" : "page";
}

function normalizeEntityType(value: string): BrainDocumentView["type"] {
  if (
    value === "person" ||
    value === "company" ||
    value === "project" ||
...
function normalizeStatus(value: string): BrainDocumentView["status"] {
...
function normalizeRelations(value: unknown) {
  if (!Array.isArray(value)) return [];
  return value.flatMap((item): BrainDocumentView["relations"] => {
...
function normalizeSources(value: unknown) {
  if (!Array.isArray(value)) return [];
  return value.flatMap((item): BrainDocumentView["sources"] => {
    if (!item || typeof item !== "object") return [];
    const record = item as Record<string, unknown>;
