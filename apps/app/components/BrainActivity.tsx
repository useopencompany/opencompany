...
import type { BrainIngestTrace } from "@opencompany/db/brain-ingest-trace";
...
type BrainActivitySourceItemsResponse = {
  sourceItems?: BrainSourceItemRow[];
};
...
function BrainActivityFeed({
  ...
  const [brainSourceItemRows, setBrainSourceItemRows] = useState<BrainSourceItemRow[]>([]);
  const sourceItems = useMemo(
    () => mergeSourceItemRows(brainSourceItemRows, (itemRows ?? []) as BrainSourceItemRow[]),
...
function mergeSourceItemRows(...sources: readonly (readonly BrainSourceItemRow[])[]) {
  const rowsById = new Map<string, BrainSourceItemRow>();
