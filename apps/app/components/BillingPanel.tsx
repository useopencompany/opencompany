...
export function BillingPanel({
  ...
  const topUpCustom = () => {
    ...
    run(() => createCreditTopUpAction(cents));
  };
  ...
  const saveAutoRefill = (enabled: boolean) => {
    ...
    startTransition(async () => {
      const result = await setAutoRefillAction({ enabled, amountCents: cents });
      if (!result.ok) {
          ...
          {data.isAdmin ? (
            data.plan === "pro" || hasManageableSubscription ? (
              <button
                type="button"
                disabled={isPending}
                onClick={() => run(createBillingPortalAction)}
              ...
              <button
                type="button"
                disabled={isPending}
                onClick={() => run(createProCheckoutAction)}
            ...
            <div className="flex flex-wrap items-center gap-2">
              {data.topUpAmountsCents.map((amountCents) => (
                <button
                  ...
                  type="button"
                  disabled={isPending}
                  onClick={() => run(() => createCreditTopUpAction(amountCents))}
