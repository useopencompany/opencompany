...
const chatActionsMock = vi.hoisted(() => ({
  ...
  setChatPinnedAction: vi.fn(async () => ({ ok: true, error: null })),
}));
...
describe("AppSidebar", () => {
  ...
  it("renders home, the brain list, and footer links", () => {
    pathnameMock.value = "/";
    const { container } = render(<AppSidebar collapsed={false} onToggleCollapsed={() => {}} />);
  ...
  it("drops a retained local working state after durable runtime reports working", async () => {
    ...
    const { rerender } = render(<AppSidebar collapsed={false} onToggleCollapsed={() => {}} />);
    ...
    await waitFor(() => {
      ...
      rerender(<AppSidebar collapsed={false} onToggleCollapsed={() => {}} />);
      expect(screen.getByTestId("sidebar-chat-unseen")).toBeInTheDocument();
    });
  });
  ...
  it("pins and unpins chats via the row toggle", async () => {
    ...
    expect(chatActionsMock.setChatPinnedAction).toHaveBeenCalledWith("goat_chat_recent", true);
    ...
    expect(chatActionsMock.setChatPinnedAction).toHaveBeenCalledWith("goat_chat_pinned", false);
  });
  ...
  it("tracks concurrent pin requests independently and restores failed rows", async () => {
    ...
    chatActionsMock.setChatPinnedAction
    ...
    expect(chatActionsMock.setChatPinnedAction).toHaveBeenCalledTimes(2);
  ...
  it("collapses to zero width and toggles via the sidebar button", () => {
    ...
    render(<AppSidebar collapsed onToggleCollapsed={onToggleCollapsed} />);
