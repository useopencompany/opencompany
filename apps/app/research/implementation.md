...
- Passed: `bun run --filter @opencompany/app test`
- Passed: `bun run typecheck`
- Passed: `bun run lint` (no errors; existing warnings remain in `apps/web`)
- Passed: `bun run format:check`
- Passed: `bun run db:generate` reported no schema changes after `0090`.
- Passed: `curl -I --max-time 5 http://127.0.0.1:3100/brain` returned `HTTP/1.1 200 OK`.
...
- Created a project, appended a first-class evidence record with `append-evidence`, linked it to
  the company, queried with graph hops, and verified query JSON included id, path context, type,
  status, compiled truth excerpt, and relation context.
