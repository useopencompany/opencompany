...
describe("toDirectEndpoint", () => {
  it("strips the -pooler label from a Neon pooled host", () => {
    expect(
      toDirectEndpoint(
        "postgresql://user:pw@ep-cool-name-123-pooler.c-3.eu-central-1.aws.neon.tech/neondb?sslmode=require",
      ),
    ).toBe(
      "postgresql://user:pw@ep-cool-name-123.c-3.eu-central-1.aws.neon.tech/neondb?sslmode=require",
    );
  });
...
describe("assertRunnerDbConfig", () => {
  ...
  it("accepts a DATABASE_URL fallback", () => {
    process.env.DATABASE_URL = "postgresql://user:pw@host-pooler.neon.tech/db";
    expect(() => assertRunnerDbConfig()).not.toThrow();
  });

  it("accepts an explicit RUNNER_DATABASE_URL override", () => {
    process.env.RUNNER_DATABASE_URL = "postgresql://user:pw@host.neon.tech/db";
    expect(() => assertRunnerDbConfig()).not.toThrow();
  });
  ...
  it("throws on a non-integer pool max", () => {
    process.env.DATABASE_URL = "postgresql://user:pw@host.neon.tech/db";
