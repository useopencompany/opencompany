...
import type { BrainToolInput, BrainToolOutput } from "@opencompany/agent/chat-ui";
import {
  BRAIN_ENTITY_TYPES,
...
import { brainToolRuns } from "@opencompany/db/product-schema";
...
// shape so it can be injected as `runBrainCli` into createProductChatToolContext.
  ...
  switch (input.toolInput.command) {
    ...
    case "get": {
      ...
      if (ids.length > MAX_GET_IDS) {
        throw new Error(`brain get accepts at most ${MAX_GET_IDS} ids.`);
      }
