...
export async function runCodexCoderTool(input: {
  ...
  const redact = createKnownSecretRedactor([codexApiKey, githubToken, githubAuthHeader]);
  ...
  const codexCommand = `cd ${shellQuote(layout.workRoot)} && mkdir -p ${shellQuote(
    codexHome,
  )} && export PATH=${CODEX_BIN_PATH}:"$PATH" && ${buildCodexCommand({
    task,
  ...
  const usage = codexHostedToolUsage({ model, summary });
  return {
