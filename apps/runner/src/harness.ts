...
function normalizeHarnessSpec(
  ...
  if (!model) {
    throw new Error("opencompany harness planner must choose a supported execution model.");
  }
  const systemPrompt = readNonEmptyString(record.systemPrompt);
  if (!systemPrompt) {
    throw new Error("opencompany harness planner must return a non-empty systemPrompt.");
  }
