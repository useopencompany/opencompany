...
describe("Goat Codex runtime tickets", () => {
  it("round-trips an owner-bound short-lived ticket", () => {
    ...
    expect(verifyGoatCodexRuntimeTicket({ ticket: signed.ticket, secret, now: 2_000 })).toEqual({
      v: 1,
      codexChatSessionId,
      userWorkosId: "user_123",
      expiresAt: 61_000,
  ...
  it("rejects expiry and tampering", () => {
    ...
    expect(verifyGoatCodexRuntimeTicket({ ticket: signed.ticket, secret, now: 61_000 })).toBeNull();
    expect(
      verifyGoatCodexRuntimeTicket({ ticket: `${signed.ticket}x`, secret, now: 2_000 }),
    ).toBeNull();
  });
});

describe("Goat Codex preview capabilities", () => {
  it("fits in one DNS label and round-trips the session, port, and expiry", () => {
    ...
    expect(
      verifyGoatCodexPreviewCapability({ capability: signed.capability, secret, now: 2_000 }),
    ).toEqual({ codexChatSessionId, port: 3_000, expiresAt: 28_801_000 });
  });
  ...
  it("rejects tampering, expiry, invalid session ids, and invalid ports", () => {
    ...
    expect(
      verifyGoatCodexPreviewCapability({ capability: tampered, secret, now: 2_000 }),
    ).toBeNull();
    expect(
      verifyGoatCodexPreviewCapability({
    ...
    expect(() =>
      createGoatCodexPreviewCapability({
        codexChatSessionId: "goat_codex_chat_not-a-uuid",
        port: 3_000,
        secret,
      }),
    ).toThrow(/valid UUID/);
    expect(() => createGoatCodexPreviewCapability({ codexChatSessionId, port: 0, secret })).toThrow(
      /Preview port/,
    );
  });
});
