...
import { createGoatClaudeActionGatewayTicket } from "@opencompany/agent-runtime";
...
describe("runner Claude actions MCP", () => {
  it("rejects a missing sandbox capability", async () => {
    ...
    const missing = await app.inject({ method: "POST", url: "/internal/goat/claude-actions" });
  ...
  it("rejects an oversized MCP body before tool dispatch", async () => {
    ...
    registerGoatClaudeActionsMcpRoute(app, env, { executeAction });
  ...
  it("initializes and reauthorizes every MCP tool operation", async () => {
    ...
    const app = Fastify();
    apps.push(app);
    registerGoatClaudeActionsMcpRoute(app, env, { authorize, executeAction });
    await app.listen({ host: "127.0.0.1", port: 0 });
    const address = app.server.address();
    if (!address || typeof address === "string") throw new Error("Expected a TCP test server.");
    const ticket = createGoatClaudeActionGatewayTicket({
      ...capability,
