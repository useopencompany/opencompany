...
export async function materializeGoatBrainForTask(input: {
  ...
  for (const document of documents) {
    ...
    await input.sandbox.commands.run(
      `mkdir -p ${shellQuote(`${GOAT_BRAIN_ROOT}/${document.folderPath}`)}`,
      { timeoutMs: 30_000 },
    );
    await input.sandbox.files.write(`${GOAT_BRAIN_ROOT}/${relativePath}`, document.content);
    files.push({
...
function validateSandboxBrainFile(file: { relativePath: string; content: string }):
  | {
      ok: true;
      brainId: string;
      folderPath: string;
...
async function ensureBrainFolder(userWorkosId: string, path: string) {
  ...
  await getDb()
    ...
    .values({
      ...
      userWorkosId,
      path,
      source: (DEFAULT_GOAT_BRAIN_FOLDERS as readonly string[]).includes(path)
        ? "system"
        : "custom",
      createdAt: now,
      updatedAt: now,
