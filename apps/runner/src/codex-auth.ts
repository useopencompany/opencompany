...
export async function startCodexDeviceAuthFlow(input: {
  ...
  try {
    ...
    logger.info("Stored pending opencompany Codex auth flow", {
    ...
    logger.warn("opencompany Codex auth flow start failed", {
...
async function supersedeActiveCodexAuthFlows(userWorkosId: string) {
  ...
  logger.info("Superseded active opencompany Codex auth flows", {
...
export async function pollCodexDeviceAuthFlow(input: {
  ...
  if (!flow) {
    logger.warn("opencompany Codex auth flow not found during poll", {
  ...
  try {
    ...
    logger.warn("opencompany Codex auth sandbox connect failed", {
  ...
  if (authJson) {
    try {
      ...
      await killSandbox(flow.sandboxId).catch(() => {});
      logger.info("opencompany Codex auth flow completed", {
      ...
      await killSandbox(flow.sandboxId).catch(() => {});
      logger.warn("opencompany Codex auth validation failed", {
...
async function createCodexAuthSandbox(input: {
  ...
  logger.debug("Creating Codex auth sandbox", {
    event: "opencompany.runner_goat_codex_auth_sandbox_create_started",
  ...
  logger.info("Codex auth sandbox created", {
    event: "opencompany.runner_goat_codex_auth_sandbox_created",
    sandbox_id: sandbox.sandboxId,
    ...input.ownerLogFields,
  });
...
async function spawnCodexDeviceLogin(sandbox: SandboxHandle) {
  ...
  logger.debug("Codex device login command started in background", {
    event: "opencompany.runner_goat_codex_auth_login_background_started",
    sandbox_id: sandbox.sandboxId,
