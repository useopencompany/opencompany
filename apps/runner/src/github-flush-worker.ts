...
import type { GitHubPullRequestEventType, IntegrationStatus } from "@opencompany/db/schema";
...
export async function listDueGitHubPullRequestWindows(input: {
  now?: Date;
  ...
  `);
  return rowsFromExecute<GitHubDueWindow>(result);
}

export async function flushGitHubPullRequestWindow(window: GitHubDueWindow): Promise<{
  sourceItemId: string;
  eventCount: number;
  enqueued: boolean;
} | null> {
...
export function buildGitHubPullRequestWindowItem(input: {
  ...
  return normalizeGitHubPullRequestWindow({
    windowId: newGitHubPullRequestWindowId(),
...
async function loadGitHubIntegrationStatus(
  ...
  const rows = rowsFromExecute<{ status: IntegrationStatus }>(
    await getDb().execute(sql`
