...
import type { WorkspaceRole } from "@opencompany/db/product-schema";
import { users } from "@opencompany/db/product-schema";
...
// Who is asking in Slack, resolved against the installing opencompany workspace.
// "member" carries the mapped opencompany user (attribution, brain access, action
...
async function resolveUncached(input: {
  ...
  // Ambiguous emails (0 or >1 opencompany accounts) never map: acting as the wrong
  ...
  // A opencompany user outside the installing workspace is treated exactly like a
