...
vi.mock("@opencompany/telemetry", async (importOriginal) => {
  ...
  return {
    ...actual,
    recordCounter: telemetry.recordCounter,
    recordHistogram: telemetry.recordHistogram,
  };
});
...
vi.mock("./opencompany-chat", () => ({
  runProductChatTurn: chatMocks.runProductChatTurn,
}));
...
describe("opencompany Codex chat worker shutdown", () => {
...
describe("terminal opencompany Codex sandbox reconciliation", () => {
...
describe("runClaimedTurn", () => {
  ...
  it("dispatches opencompany turns without coding-engine recovery state", async () => {
    ...
    expect(chatMocks.runProductChatTurn).toHaveBeenCalledWith(
      expect.objectContaining({
        session: expect.objectContaining({ engine: "opencompany" }),
        turn: expect.objectContaining({ attempts: 2 }),
      }),
    );
    expect(chatMocks.runProductChatTurn.mock.calls[0]?.[0]).not.toHaveProperty("recovery");
