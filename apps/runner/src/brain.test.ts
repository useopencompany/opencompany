...
const dbMocks = vi.hoisted(() => ({
  getDb: vi.fn(),
  getWebDb: vi.fn(() => {
    throw new Error("runner opencompany brain code must not use the web DB client");
  }),
}));
...
describe("materializeBrainToLocalRoot", () => {
  it("writes the bundled CLI and payload/sidecar docs to a local root", async () => {
    ...
    try {
      ...
      await expect(readFile(snapshot.cliPath, "utf8")).resolves.toContain("opencompany-brain");
    ...
    expect(db.insertedValues).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          userWorkosId: "user_1",
          brainId: "market-report",
          summary: "Created from opencompany task goat_task_1.",
...
async function tempRoot() {
  return mkdtemp(path.join(os.tmpdir(), "opencompany-brain-test-"));
}
