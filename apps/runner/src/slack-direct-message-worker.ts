...
export function slackDirectMessagePrompt(input: {
  ...
  return `Slack direct message\n\nSender: ${sender}\n\nThis session was started by a direct message to the opencompany Slack bot and runs on ${name}'s own account. Answer the message below.\n\nBefore writing your final task answer, call ${SLACK_BOT_TOOL_NAME} once with your Slack reply as text and "slack-dm-${input.requestId}" as messageKey. Omit channel so the tool posts to the originating thread. This thread takes exactly one reply per message, so the whole answer goes in that single call: do not split it and do not pass replyToMessageKey. The tool description describes a run announcing itself in a channel and asking for input; this is the opposite situation, so that shape does not apply. ${name} asked you a question in a direct message and your reply is the answer to it. Lead with the answer, then whatever they need to act on it. Keep the founder voice the tool asks for: spoken language, short sentences, no preamble, no headings, no internal identifiers. The assistant turn itself is not sent to Slack.\n\n--- Message begins ---\nFrom: ${sender}\n${quoteSlackText(input.text)}\n--- Message ends ---`;
}
