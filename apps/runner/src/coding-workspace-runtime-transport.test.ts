...
describe("opencompany coding workspace origin validation", () => {
  it("requires an exact configured browser origin", () => {
    const allowed = ["https://opencompany.example.com"];
    expect(isCodingWorkspaceOriginAllowed("https://opencompany.example.com", allowed)).toBe(true);
...
describe("opencompany coding workspace preview response headers", () => {
  it("rewrites upstream authority, cookie domains, and framing policy", () => {
    const headers = rewritePreviewResponseHeaders(
      ...
      "signed.preview.opencompany.example.com",
      ["https://opencompany.example.com"],
      "https",
      3_000,
    );

    expect(headers.location).toBe("https://signed.preview.opencompany.example.com/dashboard");
    expect(headers["set-cookie"]).toEqual(["session=abc; Path=/; HttpOnly"]);
    expect(headers["x-frame-options"]).toBeUndefined();
    expect(headers["content-security-policy"]).toBe(
      "default-src 'self'; frame-ancestors https://opencompany.example.com",
    );
  ...
  it("rewrites selected loopback redirects but leaves external redirects alone", () => {
    ...
    expect(loopback.location).toBe("https://signed.preview.opencompany.example.com/login");
    expect(external.location).toBe("https://accounts.example.com/login");
  });
});

describe("opencompany coding workspace preview WebSocket transport", () => {
...
describe("opencompany coding workspace terminal transport", () => {
  it.each([
...
describe("opencompany coding workspace terminal input latency", () => {
  ...
  it("flushes submitted terminal lines without waiting for the batch timer", async () => {
    vi.useFakeTimers();
    const kill = vi.fn(async () => true);
    const keystrokes: string[] = [];
    const sendInput = vi.fn(async (_pid: number, data: Uint8Array) => {
    ...
    webSocket.emit("message", Buffer.from("l"), true);
    webSocket.emit("message", Buffer.from("s\r"), true);

    await vi.waitFor(() => expect(keystrokes).toEqual(["ls\r"]), { timeout: 5_000 });
    webSocket.emit("close");
    await vi.waitFor(() => expect(kill).toHaveBeenCalledOnce(), { timeout: 5_000 });
  });
...
describe("opencompany coding workspace runtime tools install command", () => {
