...
describe("sweepDueTaskSchedules", () => {
  ...
  it("executes workflow duplicate claims against Postgres", async () => {
  ...
  it("creates a workflow task for a due workflow schedule", async () => {
    ...
    expect(sqlTextFromExecuteCall(execute, 0)).toContain("FROM goat.workflows");
    expect(sqlTextFromExecuteCall(execute, 0)).toContain("jsonb_array_elements");
    expect(sqlTextFromExecuteCall(execute, 1)).toContain("trigger_id");
