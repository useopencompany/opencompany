...
describe("opencompany chat Gateway options", () => {
  it("enables automatic prompt caching while preserving attribution", () => {
    ...
    expect(productChatGatewayProviderOptions(attribution)).toEqual({
      gateway: {
        caching: "auto",
...
describe("consumeProductChatStream", () => {
  it("separates a 50 ms presentation cadence from 500 ms durable projections", async () => {
    ...
    await consumeProductChatStream({
  ...
  it("projects an image generated by use_action as a durable artifact", async () => {
    const project = vi.fn(async (_projection: ProductChatProjection) => undefined);
    const artifact = {
      artifactId: "artifact_image_1",
      artifactVersionId: "artifact_image_version_1",
      version: 1,
      title: "Blueprint restyle",
      description: "Generated with Nano Banana.",
      ...
      fullStream: streamParts(
        {
          ...
          input: {
            ...
            params: { prompt: "Restyle the attached diagram." },
          },
        },
        {
          type: "tool-result",
          ...
          output: { ok: true, result: { artifact } },
        },
        { type: "finish", finishReason: "tool-calls" },
      ),
      sink: { project, recordStepUsage: vi.fn(async () => undefined) },
    ...
    expect(result.parts).toEqual([
      ...
      { type: "data-artifact-file", data: artifact },
    ]);
  });
  ...
  it("force-flushes the newest throttled text when the user interrupts", async () => {
    const controller = new AbortController();
    const interrupted = new ProductChatInterruptedError();
    const project = vi.fn(async (_projection: ProductChatProjection) => undefined);
    const present = vi.fn();
...
describe("opencompanyModelMessagesFromStored", () => {
  ...
  it("normalizes a legacy null approval reason before a paused Run continues", async () => {
    ...
    const modelMessages = await opencompanyModelMessagesFromStored(messages, "user_approval", {
  ...
  it("replays a trusted denial response when a paused Run continues", async () => {
    ...
    const modelMessages = await opencompanyModelMessagesFromStored(messages, "user_denial", {
      includeCurrentAssistantMessage: true,
    });
    const serialized = JSON.stringify(modelMessages);
