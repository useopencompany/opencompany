...
} from "@opencompany/agent/application/claude-tools";
...
import {
  type ClaudeActionGatewayTicketPayload,
  verifyClaudeActionGatewayTicket,
} from "@opencompany/agent-runtime";
import { createLogger } from "@opencompany/observability";
import type { FastifyInstance } from "fastify";
import { publishClaudeChatArtifact } from "./chat-artifacts";
import type { RunnerEnv } from "./env";

const MAX_MCP_BODY_BYTES = 256 * 1024;
const logger = createLogger({
  service: "opencompany-runner",
  runtime: "goat-claude-actions-mcp",
});

type ClaudeMcpDependencies = {
  authorize: typeof authorizePersistedClaudeToolCapability;
  executeAction: ClaudeActionToolDependencies["executeAction"];
  publishArtifact: typeof publishClaudeChatArtifact;
};

const defaultDependencies: ClaudeMcpDependencies = {
  authorize: authorizePersistedClaudeToolCapability,
  executeAction: executeActionGateway,
  publishArtifact: publishClaudeChatArtifact,
};

export function registerClaudeActionsMcpRoute(
  app: FastifyInstance,
  env: RunnerEnv,
...
function verifiedCapability(
  ...
  env: RunnerEnv,
): ClaudeActionGatewayTicketPayload | null {
  const ticket = typeof header === "string" ? header : "";
  if (!ticket || ticket.length > 4_096) return null;
  const payload = verifyClaudeActionGatewayTicket({ ticket, secret: env.internalToken });
