...
import {
  GOAT_BRAIN_WORKER_ADMISSION_CHANNEL,
  type GoatBrainWorker,
  parseGoatBrainWorkerAdmission,
...
export type GoatBrainWorkerAdmissionCallbacks = Record<GoatBrainWorker, () => void>;

export function startGoatBrainWorkerAdmissionListener(input: {
  pool: Pool;
  callbacks: GoatBrainWorkerAdmissionCallbacks;
  ...
  const onNotification = (notification: PooledDbNotification) => {
    if (notification.channel !== GOAT_BRAIN_WORKER_ADMISSION_CHANNEL || !notification.payload) {
      return;
    }
    const worker = parseGoatBrainWorkerAdmission(notification.payload);
  ...
  return {
    stop: async () => {
      ...
      current.removeListener("notification", onNotification);
      current.removeListener("error", onError);
      await current.query(`UNLISTEN ${GOAT_BRAIN_WORKER_ADMISSION_CHANNEL}`).catch(() => undefined);
      current.release();
    },
  };
}
