...
### Added
- start a workflow the moment jamie finishes a meeting, with the summary, attendees, and action items as context. create a webhook in jamie pointing at the endpoint shown in the plugin's events section, save the key jamie gives you, turn on meeting completed, then choose it as your workflow's event. filter to meetings with guests from outside your company, or to internal ones only — @louis.
...
- GPT-6 Astra Codex sessions now start successfully instead of failing before the first response because the sandbox runtime did not yet recognize the model (#1628) — @louis.
