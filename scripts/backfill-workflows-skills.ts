...
// own workspace-scoped tables (migration 0164); this copies existing content
...
//
// Usage: DATABASE_URL=postgres://... bun run scripts/backfill-workflows-skills.ts

import { createHash, randomUUID } from "node:crypto";
import {
  brainSkillFromDocument,
  brainWorkflowFromDocument,
  parseBrainDocument,
} from "@opencompany/brain";
import { getDb } from "@opencompany/db/client";
import { brainDocuments, brains, skills, workflows } from "@opencompany/db/product-schema";
...
async function main() {
  ...
  const brains = await db
    .select({ id: brains.id, workspaceId: brains.workspaceId, slug: brains.slug })
    .from(brains);
...
async function backfillWorkflows(db: ReturnType<typeof getDb>, brainMeta: Map<string, BrainMeta>) {
  const docs = await db
    .select({
      ...
      format: brainDocuments.format,
    })
    .from(brainDocuments)
    .where(
      or(
        eq(brainDocuments.folderPath, "workflows"),
        like(brainDocuments.folderPath, "workflows/%"),
      ),
    );
  ...
  const parsed = generalFirst(
    docs.flatMap((row) => {
      ...
      const workflow = brainWorkflowFromDocument(parseBrainDocument(row.content));
...
async function backfillSkills(db: ReturnType<typeof getDb>, brainMeta: Map<string, BrainMeta>) {
  const docs = await db
    ...
    .from(brainDocuments)
    .where(
      or(eq(brainDocuments.folderPath, "skills"), like(brainDocuments.folderPath, "skills/%")),
    );
  ...
  const parsed = generalFirst(
    docs.flatMap((row) => {
      const meta = brainMeta.get(row.brainRef);
      if (!meta || row.format !== "markdown") return [];
      const skill = brainSkillFromDocument(parseBrainDocument(row.content));
  ...
  const existing = await db
    .select({ id: skills.id, workspaceId: skills.workspaceId, slug: skills.slug })
    .from(skills);
  const existingIds = new Set(existing.map((row) => row.id));
  const takenByWorkspace = new Map<string, Set<string>>();
  for (const row of existing) {
