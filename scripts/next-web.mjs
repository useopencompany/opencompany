...
if (args[0] === "dev") {
  env.PORT = env.OPENCOMPANY_PORT?.trim() || "3002";
  const hasPortArg = args.includes("-p") || args.includes("--port");
  if (!hasPortArg) {
    args.push("--port", env.PORT);
  }

  const appUrl = env.OPENCOMPANY_NEXT_PUBLIC_APP_URL?.trim() || `http://localhost:${env.PORT}`;
  env.NEXT_PUBLIC_APP_URL = appUrl;
  env.NEXT_PUBLIC_WORKOS_REDIRECT_URI =
    env.OPENCOMPANY_NEXT_PUBLIC_WORKOS_REDIRECT_URI?.trim() || `${appUrl}/auth/callback`;
  env.WORKOS_REDIRECT_URI ||= env.NEXT_PUBLIC_WORKOS_REDIRECT_URI;
} else {
  if (env.OPENCOMPANY_NEXT_PUBLIC_APP_URL?.trim()) {
    env.NEXT_PUBLIC_APP_URL = env.OPENCOMPANY_NEXT_PUBLIC_APP_URL.trim();
  }
  if (env.OPENCOMPANY_NEXT_PUBLIC_WORKOS_REDIRECT_URI?.trim()) {
    env.NEXT_PUBLIC_WORKOS_REDIRECT_URI = env.OPENCOMPANY_NEXT_PUBLIC_WORKOS_REDIRECT_URI.trim();
    env.WORKOS_REDIRECT_URI ||= env.NEXT_PUBLIC_WORKOS_REDIRECT_URI;
  }
}
