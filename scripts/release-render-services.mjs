...
import { renderSurfaceConfig, renderSurfaceResults } from "./lib/release-orchestration.mjs";
...
async function deployAndSmoke(surface) {
  const config = renderSurfaceConfig(surface);
  const serviceId = requiredEnv(config.serviceIdEnv);
  await run(
    "bun",
    ["run", "release:render", "--", `--expected-shutdown-delay=${config.maxShutdownDelaySeconds}`],
    {
      ...process.env,
