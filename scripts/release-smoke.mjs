...
if (checkWeb && !webUrl && !webVercelDeployment) {
  console.error(
    "OPENCOMPANY_URL, PRODUCTION_OPENCOMPANY_URL, or SMOKE_WEB_VERCEL_DEPLOYMENT is required.",
  );
  process.exit(1);
}
...
async function checkUntilReady(name, url, maxAttempts, waitMs) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      if (name === "runner" && payload.capabilities?.acpToolsMcp !== "v3") {
        throw new Error("runner does not advertise the v3 ACP tools MCP transport");
      }
