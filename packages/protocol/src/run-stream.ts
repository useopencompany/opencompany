...
export class RunEventCursorError extends Error {
  constructor(
    readonly cursorKind: "durable" | "presentation",
    cause: unknown,
  ) {
    super(`The saved ${cursorKind} cursor is invalid.`, { cause });
    this.name = "RunEventCursorError";
  }
}

function validateCursor(kind: "durable" | "presentation", cursor: string | undefined): void {
  if (!cursor) return;
  try {
    if (kind === "durable") decodeEventCursor(cursor);
    else decodePresentationCursor(cursor);
  } catch (error) {
    throw new RunEventCursorError(kind, error);
  }
}
...
export async function* streamRunEvents(
  ...
  validateCursor("durable", options.cursor);
  validateCursor("presentation", options.presentationCursor);
  ...
  while (!options.signal?.aborted) {
    ...
    try {
      ...
      if (!(options.isRetryableError?.(error) ?? error instanceof TypeError)) throw error;
      options.onReconnect?.();
