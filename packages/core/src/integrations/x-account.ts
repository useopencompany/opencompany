...
const X_ACCOUNT_INTEGRATION_ENVS = ["X_CLIENT_ID", "X_CLIENT_SECRET", "X_STATE_SECRET"] as const;
...
export function buildXAccountAuthorizationUrl(state: string, codeChallenge: string) {
  ...
  url.searchParams.set("client_id", requiredEnv("X_CLIENT_ID"));
...
function signStateBody(body: string) {
  return createHmac("sha256", requiredEnv("X_STATE_SECRET")).update(body).digest("base64url");
}
...
function requiredEnv(name: string) {
  ...
  if (!value) throw new Error(`${name} is required for the X integration.`);
  return value;
}
