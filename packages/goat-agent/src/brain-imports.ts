...
import {
  ...
  startGoatBrainImportRunIdempotent,
...
import type { GoatBrainSourceApplicationService } from "./brain-sources";
...
export type GoatBrainImportSelectionInput = Record<
  string,
  {
    enabled: boolean;
...
export type GoatBrainImportCommandResult = {
  importRunId: string;
  status: GoatBrainImportStatus;
  replayed: boolean;
};

export class GoatBrainImportApplicationService {
  constructor(
    private readonly db: DbLike,
    private readonly brainSources: Pick<GoatBrainSourceApplicationService, "list" | "listOptions">,
  ) {}
  ...
  async start(
    ...
    input: {
      ...
      companyUrl: string;
      focus?: string;
      sourceSelection: GoatBrainImportSelectionInput;
    },
  ): Promise<GoatBrainImportCommandResult> {
    const id = await this.authorizeAdminBrain(actor, brainId);
    let company: { url: string; domain: string };
    try {
      company = normalizeGoatCompanyUrl(input.companyUrl);
    } catch (error) {
      throw new CoreError(
        "invalid_argument",
    ...
    const result = await startGoatBrainImportRunIdempotent({
      actor,
      brainRef: id,
  ...
  async cancel(
    ...
    const id = await this.authorizeAdminBrain(actor, brainId);
    const runId = resourceId(importRunId, "importRunId");
    await cancelGoatBrainImport({ importRunId: runId, brainRef: id, db: this.db });
  ...
  async retry(
    ...
    const id = await this.authorizeAdminBrain(actor, brainId);
    const runId = resourceId(importRunId, "importRunId");
    await retryGoatBrainImportDiscovery({ importRunId: runId, brainRef: id, db: this.db });
  ...
  private async validateSelection(
    actor: Actor,
    brainId: string,
    selection: GoatBrainImportSelectionInput,
...
function integrationIdFor(
  details: Awaited<ReturnType<GoatBrainSourceApplicationService["list"]>>,
...
function sanitizeEnabledProviders(value: GoatBrainImportProvider[]): GoatBrainImportProvider[] {
