...
// App-specific pieces injected by the caller: draft-id allocation against the
// brain's current file inventory, and the ingest-worker wake call.
export type GoatBrainCaptureDeps = {
  nextAvailableBrainId: (brainRef: string, baseId: string) => Promise<string>;
  wakeIngest: () => Promise<void>;
};
...
export async function captureToGoatBrainInbox(
  ...
  const draftBrainId = await deps.nextAvailableBrainId(input.brainRef, normalizeGoatBrainId(title));
