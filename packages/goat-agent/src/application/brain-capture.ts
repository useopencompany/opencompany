...
import type { GoatCodexBrainCaptureGatewayResponse } from "@opencompany/agent-runtime";
import type { GoatStoredChatMessage, SaveToBrainToolOutput } from "../chat-ui";

export type GoatBrainCaptureContext = {
  actorId: string;
  workspaceId: string;
...
export type GoatBrainCaptureCommand = {
  sessionId: string;
  runId: string;
  content?: string;
  title?: string;
...
export type GoatBrainCaptureServiceDependencies = {
  loadContext: (command: GoatBrainCaptureCommand) => Promise<GoatBrainCaptureContext | null>;
  ...
  loadMessages: (
    conversationId: string,
  ) => Promise<readonly Pick<GoatStoredChatMessage, "role" | "attachments">[]>;
  ...
  onError?: (error: unknown, command: GoatBrainCaptureCommand) => void;
};

export async function executeGoatBrainCaptureService(input: {
  command: GoatBrainCaptureCommand;
  dependencies: GoatBrainCaptureServiceDependencies;
