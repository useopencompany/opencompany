...
export type GoatClaudeToolContext = {
  sessionId: string;
  runId: string;
  signal?: AbortSignal;
};

export type GoatClaudeActionToolDependencies = {
  executeAction: (input: {
...
export function registerGoatClaudeActionServiceTools(
  server: McpServer,
  ctx: GoatClaudeToolContext,
  dependencies: GoatClaudeActionToolDependencies,
) {
  server.registerTool(
    GOAT_PUBLISH_ARTIFACT_TOOL_NAME,
    {
