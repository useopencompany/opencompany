...
const capability: GoatClaudeToolCapability = {
...
function state(
  overrides: Partial<GoatClaudeToolAuthorityState> = {},
): GoatClaudeToolAuthorityState {
  return {
    sessionId: "session_1",
    turnId: "run_1",
    attemptId: "attempt_1",
...
describe("Claude tool capability authority", () => {
  it("authorizes only the active persisted Claude attempt", () => {
    expect(authorizeGoatClaudeToolCapability({ capability, state: state(), now })).toEqual({
      actorId: "user_1",
      workspaceId: "workspace_1",
      conversationId: "conversation_1",
      sandboxId: "sandbox_1",
  ...
  it.each([
    ...
    expect(
      authorizeGoatClaudeToolCapability({ capability, state: state(overrides), now }),
    ).toBeNull();
  });
});
