...
// App-specific check injected by the caller: whether a successful run should be
// recorded as completing the user's MCP setup (see apps/goat lib/mcp-setup.ts).
export type GoatBrainMcpSetupCompletionCheck = (input: {
...
export async function runGoatBrainCliForUser(input: {
  ...
  signal?: AbortSignal;
  completesMcpSetup?: GoatBrainMcpSetupCompletionCheck;
...
export async function runGoatBrainToolForUser(input: {
  ...
  return runGoatBrainCliForUser({
    ...
    ...(input.completesMcpSetup ? { completesMcpSetup: input.completesMcpSetup } : {}),
  });
}
...
async function runResolvedGoatBrainCliForUser(
  ...
  await recordGoatBrainToolRun({
    ...
    syncResult,
    completesMcpSetup: input.completesMcpSetup ?? null,
  });
  return output;
}
...
async function recordGoatBrainToolRun(input: {
  ...
  completesMcpSetup: GoatBrainMcpSetupCompletionCheck | null;
}) {
  try {
    const db = getDb();
    ...
    const completesMcpSetup = input.completesMcpSetup
      ? input.completesMcpSetup({
