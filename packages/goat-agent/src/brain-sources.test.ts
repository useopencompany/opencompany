...
vi.mock("@opencompany/analytics/goat/server", () => ({
  captureGoatServerEvent: mocks.captureEvent,
}));
vi.mock("@opencompany/db/goat-brain-sources", () => ({
  deleteGoatBrainSource: mocks.deleteSource,
  hasAnyBrainSourceForIntegration: mocks.hasAnySource,
  listGoatBrainSourcesForBrain: mocks.listSources,
  listGoatPersonalIntegrationAccounts: mocks.listPersonalAccounts,
  setGoatBrainSourceEnabled: mocks.setEnabled,
  upsertGoatBrainSource: mocks.upsertSource,
}));
vi.mock("@opencompany/db/goat-workspaces", () => ({
  getDefaultGoatBrainForUser: mocks.getDefaultBrain,
  getGoatBrainAccess: mocks.getBrainAccess,
}));
vi.mock("@opencompany/db/goat-github", async (importActual) => ({
  ...(await importActual<typeof import("@opencompany/db/goat-github")>()),
  listGoatGitHubIntegrationRepositories: mocks.listGitHubRepositories,
}));
vi.mock("@opencompany/db/goat-google-drive", async (importActual) => ({
  ...(await importActual<typeof import("@opencompany/db/goat-google-drive")>()),
  upsertGoatGoogleDriveSyncCursor: mocks.upsertCursor,
}));
vi.mock("./app-url", () => ({ getGoatAppUrl: () => "https://app.example.com" }));
vi.mock("./integrations/google-drive-source", async (importActual) => ({
  ...
  getGoatGoogleDriveFile: mocks.getDriveFile,
  getGoatGoogleDriveStartPageToken: mocks.getDriveToken,
  listGoatGoogleDriveFiles: mocks.listDriveFiles,
  listGoatGoogleSharedDrives: mocks.listSharedDrives,
  loadOwnGoatGoogleDriveAccount: mocks.loadDriveAccount,
}));

const { GoatBrainSourceApplicationService } = await import("./brain-sources");
const { GoatGoogleDriveReconnectRequiredError } = await import(
  "./integrations/google-drive-source"
);
...
describe("GoatBrainSourceApplicationService", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("lets a member toggle their own personal source", async () => {
    const service = new GoatBrainSourceApplicationService(
      queuedDb([
        {
          id: "source_1",
          provider: "slack",
  ...
  it("hides a Brain when canonical access resolves it in another workspace", async () => {
    ...
    const db = queuedDb();
    const service = new GoatBrainSourceApplicationService(db);
  ...
  it("makes removal idempotent when no source row exists", async () => {
    const service = new GoatBrainSourceApplicationService(queuedDb([]));
  ...
  it("maps expired Drive credentials to an actionable conflict", async () => {
    mocks.getDriveFile.mockRejectedValueOnce(new GoatGoogleDriveReconnectRequiredError());
    const service = new GoatBrainSourceApplicationService(
      queuedDb(
        [
          {
