...
// nothing else, the same way createCodingWorkspaceTicket scopes preview access.
...
export function verifyClaudeActionGatewayTicket(input: {
  ...
  try {
    ...
    ) as Partial<ClaudeActionGatewayTicketPayload>;
    if (
    ...
    return value as ClaudeActionGatewayTicketPayload;
  } catch {
    return null;
  }
}
