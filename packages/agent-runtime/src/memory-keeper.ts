...
export const MEMORY_KEEPER_SYSTEM_PROMPT = [
  ...
  "   - agent/user.md, the agent's profile, is a cache of memory, not a second store: the few things the agent needs in every turn (identity, preferences, communication style, goals). Promote into it what earns its place, demote what no longer does — it loads into every future session (~3KB cap), and it's fine for promoted facts to live in both places.",
