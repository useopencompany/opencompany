...
export function resolveAgentRuntimeConfig(input: {
  ...
  const context = [
    ...
    isPersonal
      ? "The sandbox has these file roots. Choose where to put something by how long it should last: ./work is a temporary scratch directory for this session only (drafts, intermediate files, deliverables, cloned repos) — nothing here survives the session. personal-brain/ is the user's private, persistent knowledge space — their notes, research, documents, decisions, and saved artifacts; it persists across sessions and is the user's to keep and grow. memory/ is your own persistent structured memory (see below). ./agent is your private agent folder, including your profile (agent/user.md) and your personal skills. ./skills is read-only; open it with read_skill."
    ...
    isPersonal
      ? "memory/ is your structured, evidence-grounded MEMORY (canonical objects + cited evidence) — your distilled understanding of the user, built from their personal brain and your conversations: durable facts, preferences, priorities, patterns (specific people, companies, projects, decisions, lessons). It is distinct from personal-brain/, which holds the user's own raw files; memory/ is your compiled view of what matters. Manage it ONLY through the `memory` tool (not shell), never by editing files there directly; read the memory skill (skill id `memory`) with read_skill before using it."
    ...
    isPersonal
      ? "Where things go (route by lifespan + audience): one-off that won't matter next session → leave it in the conversation, write nothing. How you behave going forward (a standing preference, tone, default tool/model) → your .agent definition via self-edit, not a file. Who your user is (identity, preferences, communication style, goals) → agent/user.md, your profile (see the 'Your profile' section below). Any other durable fact you learn about the user (specific people, companies, projects, decisions, lessons, conventions) → structured memory via the `memory` tool. The user's own notes, research, documents, and saved artifacts → personal-brain/. A repeatable procedure worth reusing → a personal skill under agent/skills/. Throwaway scratch for this session → ./work."
