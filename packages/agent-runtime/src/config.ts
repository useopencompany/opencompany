import { EXPERIMENT_KEYS, isExperimentEnabled, type WorkspaceExperiments } from "./experiments";
import {
...
export function resolveAgentRuntimeConfig(input: {
  ...
  // Per-workspace beta flags. Gates experiment-only behaviour (e.g. the explore sub-agent tool
  // and its system-prompt guidance). Missing/undefined reads as every experiment off.
  ...
  const exploreEnabled = isExperimentEnabled(input.experiments, EXPERIMENT_KEYS.explore);
