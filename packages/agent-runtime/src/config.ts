...
export function resolveAgentRuntimeConfig(input: {
  ...
  const context = [
    ...
    isPersonal
      ? "The sandbox has these file roots. Choose where to put a file by who it is for, not by guessing how long it will matter: ./work is your private scratch space for files only you need to get the job done (cloned repos, temp data, intermediate transforms) — the user cannot see it and nothing here survives the session. personal-brain/ is the user's private, persistent knowledge space — their notes, research, documents, decisions, and everything you produce for them; files there persist across sessions and surface to the user in the app. memory/ is your own persistent structured memory (see below). ./agent is your private agent folder, including your profile (agent/user.md) and your personal skills. ./skills is read-only; open it with read_skill. This session has NO company brain/ root — brain/ does not exist here; persistent user content goes to personal-brain/, never brain/. work/ is ephemeral scratch: it is wiped when the session ends and the user can never see it, so never leave anything you want to keep — or anything the user asked you to produce — in work/."
    ...
    isPersonal
      ? "Where things go (route by audience): anything you produce for the user to keep or read — a doc, report, plan, analysis — → personal-brain/, even while you're still iterating on it; it's the only place they can open it (work/ is invisible to them and gone after the session). When unsure, prefer personal-brain — clutter is prunable, a vanished file isn't. Files only you need to do the job → ./work. One-off that won't matter next session → leave it in the conversation, write nothing. How you behave going forward (a standing preference, tone, default tool/model) → your .agent definition via self-edit, not a file. Every durable fact you learn (specific people, companies, projects, decisions, lessons, conventions) → structured memory via the `memory` tool — memory is the canonical store, always correct to write to. agent/user.md is not a second store: it's your always-loaded digest of memory (see the 'Your profile' section below) — promote the few things you need in every turn, and it's fine for them to live in both places. A repeatable procedure worth reusing → a personal skill under agent/skills/."
