...
describe("agent schedules", () => {
  test("generates and parses supported preset cron expressions", () => {
    ...
    expect(cronForSchedulePreset({ kind: "hours", interval: 6 })).toBe("0 0,6,12,18 * * *");
    expect(cronForSchedulePreset({ kind: "hours", interval: 7 })).toBe("0 0,6,12,18 * * *");
    ...
    expect(schedulePresetFromCron("0 0,6,12,18 * * *")).toEqual({ kind: "hours", interval: 6 });
    expect(schedulePresetFromCron("0 */6 * * *")).toEqual({ kind: "hours", interval: 6 });
    expect(schedulePresetFromCron("0 */7 * * *")).toBeNull();
