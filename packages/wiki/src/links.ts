...
export type WikiInlineLinkKind = "page" | "source";

export type WikiInlineLink = {
  raw: string;
  kind: WikiInlineLinkKind;
  target: string;
  label: string;
  index: number;
  valid: boolean;
...
export function parseWikiInlineLinks(text: string): WikiInlineLink[] {
  const links: WikiInlineLink[] = [];
...
export function formatWikiSourceLink(ref: string, label?: string): string {
  const normalizedTarget = ref.trim();
  if (!isValidWikiSourceRef(normalizedTarget)) throw new Error("Invalid wiki source link target.");
...
export function wikiSourceRefTargets(text: string): string[] {
  return inlineLinkTargets(text, "source");
}

export function isValidWikiInlineLinkTarget(kind: WikiInlineLinkKind, target: string): boolean {
  return isValidInlineLinkTarget(kind, target);
}

function withLabel(openedLink: string, label: string | undefined): string {
  const normalizedLabel = label?.trim();
  if (!normalizedLabel) return `${openedLink}]]`;
  if (!isValidInlineLinkLabel(normalizedLabel)) throw new Error("Invalid wiki link label.");
  return `${openedLink}|${normalizedLabel}]]`;
}

function parseBracketTarget(value: string): { kind: WikiInlineLinkKind; target: string } {
  const separator = value.indexOf(":");
  if (separator === -1) return { kind: "page", target: value };

  const prefix = value.slice(0, separator);
  const target = value.slice(separator + 1).trim();
  if (prefix === "page") return { kind: "page", target };
  if (prefix === "source") return { kind: "source", target };
  return { kind: "page", target: value };
}

function inlineLinkTargets(text: string, kind: WikiInlineLinkKind): string[] {
  const seen = new Set<string>();
  const targets: string[] = [];
  for (const link of parseWikiInlineLinks(text)) {
