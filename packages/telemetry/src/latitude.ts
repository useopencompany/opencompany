 ...
 * pipeline registered globally by telemetry.
 */
...
export async function flushLatitude(): Promise<void> {
  ...
  try {
    ...
    console.warn("[opencompany-observability] Latitude telemetry flush failed.", error);
  }
}
...
function getLatitudeClient(): Latitude | null {
  ...
  try {
    ...
    console.warn(
      "[opencompany-observability] Latitude telemetry init failed; continuing without it.",
      error,
    );
    cached = { client: null };
  }
