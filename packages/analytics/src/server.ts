...
type ModelSpendBillingSource = AnalyticsEventProperties<"model_spend_recorded">["billing_source"];
type ModelSpendEngine = AnalyticsEventProperties<"model_spend_recorded">["engine"];
type ModelSpendSurface = AnalyticsEventProperties<"model_spend_recorded">["surface"];

export type CaptureTaskSpawnedInput = {
  userWorkosId: string;
  ...
  trigger?: TaskSpawnTrigger | undefined;
};

export type LlmUsageAnalyticsSurface = "chat" | "task";

export type LlmUsageAnalyticsStage = "generation" | "routing" | "planner" | "execution" | "closer";
...
export function captureServerEvent<EventName extends AnalyticsEventName>(
  ...
  const personProperties = person ? analyticsPersonProperties(person) : undefined;

  return capturePostHogServerEvent(getPostHogConfig(), {
    event,
    distinctId,
    properties: {
      ...properties,
...
export function captureTaskSpawned(input: CaptureTaskSpawnedInput) {
  ...
  const taskOrigin = taskSpawnOrigin(hasWorkflow);
  const taskKind = taskSpawnKind({ origin: taskOrigin, trigger });
...
function taskSpawnOrigin(hasWorkflow: boolean): TaskSpawnOrigin {
  return hasWorkflow ? "workflow" : "adhoc";
}

function taskSpawnKind(input: {
  origin: TaskSpawnOrigin;
  trigger: TaskSpawnTrigger;
}): TaskSpawnKind {
...
export function captureModelSpendRecorded(input: {
  userWorkosId: string;
  workspaceId?: string | null;
  billingSource: ModelSpendBillingSource;
  surface: ModelSpendSurface;
  model: string;
  ...
  return captureServerEvent("model_spend_recorded", input.userWorkosId, {
...
export type LlmUsageRecordedAnalyticsInput = {
  distinctId: string;
  workspaceId?: string | null | undefined;
  surface: LlmUsageAnalyticsSurface;
  stage: LlmUsageAnalyticsStage;
  ...
  engine: AnalyticsEngine;
  usageSource?: AnalyticsUsageSource | undefined;
  inputTokens: number;
...
export function captureLlmUsageRecorded(input: LlmUsageRecordedAnalyticsInput) {
  return captureServerEvent("llm_usage_recorded", input.distinctId, {
    ...(input.workspaceId ? { workspace_id: input.workspaceId } : {}),
    surface: input.surface,
    stage: input.stage,
    ...
    engine: input.engine,
    usage_source: input.usageSource ?? analyticsUsageSourceForEngine(input.engine),
...
export function analyticsUsageSourceForEngine(engine: AnalyticsEngine): AnalyticsUsageSource {
  return engine === "opencompany" ? "owned_platform" : "external_harness";
}

function usageSourceForOptionalEngine(
  engine: AnalyticsEngine | undefined,
): AnalyticsUsageSource | undefined {
  return engine ? analyticsUsageSourceForEngine(engine) : undefined;
}
