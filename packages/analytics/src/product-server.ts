import type {
  ...
  ProductAnalyticsUsageSource,
  ProductTaskSpawnKind,
  ProductTaskSpawnOrigin,
  ProductTaskSpawnTrigger,
} from "./product-events";
...
type ProductModelSpendBillingSource =
  ProductAnalyticsEventProperties<"model_spend_recorded">["billing_source"];
type ProductModelSpendEngine = ProductAnalyticsEventProperties<"model_spend_recorded">["engine"];
type ProductModelSpendSurface = ProductAnalyticsEventProperties<"model_spend_recorded">["surface"];

export type CaptureProductTaskSpawnedInput = {
  userWorkosId: string;
...
export function captureProductServerEvent<EventName extends ProductAnalyticsEventName>(
  event: EventName,
  distinctId: string,
  properties: ProductAnalyticsEventProperties<EventName>,
  person?: ProductAnalyticsPerson,
) {
  const personProperties = person ? productAnalyticsPersonProperties(person) : undefined;

  return capturePostHogServerEvent(getProductPostHogConfig(), {
    event,
    distinctId,
    properties: {
      ...properties,
...
export function captureProductTaskSpawned(input: CaptureProductTaskSpawnedInput) {
  ...
  const taskOrigin = productTaskSpawnOrigin(hasWorkflow);
  const taskKind = productTaskSpawnKind({ origin: taskOrigin, trigger });

  return captureProductServerEvent(
    "task_spawned",
    input.userWorkosId,
    {
...
function productTaskSpawnOrigin(hasWorkflow: boolean): ProductTaskSpawnOrigin {
  return hasWorkflow ? "workflow" : "adhoc";
}

function productTaskSpawnKind(input: {
  origin: ProductTaskSpawnOrigin;
  trigger: ProductTaskSpawnTrigger;
}): ProductTaskSpawnKind {
...
export function captureProductModelSpendRecorded(input: {
  userWorkosId: string;
  workspaceId?: string | null;
  billingSource: ProductModelSpendBillingSource;
  surface: ProductModelSpendSurface;
  model: string;
  ...
  stage?: string;
  engine?: ProductModelSpendEngine;
  usageSource?: ProductAnalyticsUsageSource;
  ...
  return captureProductServerEvent("model_spend_recorded", input.userWorkosId, {
...
export type ProductLlmUsageRecordedAnalyticsInput = {
  distinctId: string;
  workspaceId?: string | null | undefined;
  surface: ProductLlmUsageAnalyticsSurface;
  stage: ProductLlmUsageAnalyticsStage;
...
export function captureProductLlmUsageRecorded(input: ProductLlmUsageRecordedAnalyticsInput) {
  return captureProductServerEvent("llm_usage_recorded", input.distinctId, {
    ...(input.workspaceId ? { workspace_id: input.workspaceId } : {}),
    surface: input.surface,
    stage: input.stage,
...
export function productAnalyticsUsageSourceForEngine(
  engine: ProductAnalyticsEngine,
): ProductAnalyticsUsageSource {
...
function usageSourceForOptionalEngine(
  engine: ProductAnalyticsEngine | undefined,
): ProductAnalyticsUsageSource | undefined {
  return engine ? productAnalyticsUsageSourceForEngine(engine) : undefined;
}
