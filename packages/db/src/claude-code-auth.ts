...
export async function saveClaudeCodeCredential(input: {
  ...
  if (!credential) throw new Error("Could not persist opencompany Claude Code credential.");
  return credential;
}
...
export async function loadClaudeCodeCredential(input: {
  ...
  if (credential.userWorkosId !== input.userWorkosId) {
    throw new Error("opencompany Claude Code credential row did not match the requested user.");
  }
...
export async function loadClaudeCodeAuthStatus(input: {
  db: ClaudeCodeAuthDb;
  userWorkosId: string;
}): Promise<ClaudeCodeAuthStatus | null> {
...
function decryptAuthJson(
  ...
  if (encryptedAuthJson.algorithm !== ENCRYPTION_ALGORITHM) {
    throw new Error(
      `Unsupported opencompany Claude Code credential encryption algorithm ${encryptedAuthJson.algorithm}.`,
    );
  }
  ...
  try {
    ...
    if (error instanceof UnsupportedKeyVersionError) {
      throw new Error(
        `Unsupported opencompany Claude Code credential encryption key version ${keyVersion}.`,
      );
    }
    throw error;
  }
  ...
  try {
    ...
    throw new Error("opencompany Claude Code credential could not be decrypted.");
  }
  if (typeof decrypted.token !== "string" || !decrypted.token) {
    throw new Error("opencompany Claude Code credential payload is missing its token.");
  }
