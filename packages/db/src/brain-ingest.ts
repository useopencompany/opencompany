...
export async function upsertBrainSourceItemAndEnqueue(input: {
  ...
  if (!sourceItem) throw new Error("Could not persist opencompany Brain source item.");
...
function brainIngestJobTargetWhere(input: {
  ...
  if (!brainRefWhere)
    throw new Error("Cannot build opencompany Brain ingest job target without brains.");
  ...
  if (!targetWhere) throw new Error("Cannot build opencompany Brain ingest job target.");
  return targetWhere;
}
...
function rawEventCountForWorkspace(input: {
  ...
  for (const brain of input.brainRows) {
    ...
    if (!brainKeys) {
      throw new Error(`Missing claimed event keys for opencompany Brain ${brain.id}.`);
    }
  ...
  if (keys.size < 1 || keys.size > 200) {
    throw new Error("opencompany ingestion workspace event count must be between 1 and 200.");
  }
  return keys.size;
}
