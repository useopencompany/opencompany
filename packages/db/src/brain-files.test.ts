...
beforeEach(async () => {
  root = await mkdtemp(path.join(os.tmpdir(), "opencompany-brain-files-"));
});
...
describe("opencompany Brain file sync", () => {
  ...
  it("projects compiled truth without a duplicate leading title heading", () => {
    ...
    expect(projection.body).toBe("Acme evaluates opencompany Brain.");
    expect(parseBrainDocument(projection.content).compiledTruth).toBe(projection.body);
  });
  ...
  it("recovers sidecar-backed markdown when only the payload hash is stale", async () => {
    await writeSidecarBackedPayload({
      ...
      payload: "Acme now evaluates opencompany Brain.",
      sidecar: {
    ...
    expect(files[0]?.content).toContain("Acme now evaluates opencompany Brain.");
  });
  ...
  it("reports newly created page descriptors from sync", async () => {
    const content = createBrainMarkdownContent({
      ...
      type: "company",
      status: "draft",
      compiledTruth: "Acme is evaluating opencompany Brain.",
    });
...
describe("opencompany Brain asset projections", () => {
