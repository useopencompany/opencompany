...
import { startGoatBrainImportRunIdempotent } from "./goat-brain-import";
...
describe("startGoatBrainImportRunIdempotent", () => {
  let database: PGlite;
  ...
  it("creates one run and replays the same command durably", async () => {
    ...
    const created = await startGoatBrainImportRunIdempotent(command);
    const replay = await startGoatBrainImportRunIdempotent(command);
  ...
  it("rejects key reuse for a different normalized command", async () => {
    ...
    await startGoatBrainImportRunIdempotent(command);

    await expect(
      startGoatBrainImportRunIdempotent({ ...command, companyUrl: "other.com" }),
  ...
  it("keeps one active import per brain the storage invariant", async () => {
    ...
    await startGoatBrainImportRunIdempotent({ ...base, idempotencyKey: "import-1" });

    await expect(
      startGoatBrainImportRunIdempotent({ ...base, idempotencyKey: "import-2" }),
    ...
    const next = await startGoatBrainImportRunIdempotent({
      ...base,
  ...
  it("recovers a reservation whose run insert was lost", async () => {
    ...
    const recovered = await startGoatBrainImportRunIdempotent(command);
  ...
  it("scopes the same idempotency key independently per actor and Workspace", async () => {
    const first = await startGoatBrainImportRunIdempotent({
      actor: actor(),
      brainRef: "brain_1",
    ...
    const second = await startGoatBrainImportRunIdempotent({
