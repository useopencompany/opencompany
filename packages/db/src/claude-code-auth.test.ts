...
describe("opencompany Claude Code credential validation state", () => {
  afterEach(() => {
    vi.unstubAllEnvs();
  });
  ...
  it("reads status fields without touching the encrypted payload", async () => {
    ...
    await expect(
      loadClaudeCodeAuthStatus({ db: db as never, userWorkosId: "user_123" }),
    ...
    await expect(
      loadClaudeCodeAuthStatus({ db: empty as never, userWorkosId: "user_123" }),
    ).resolves.toBeNull();
  });
});
