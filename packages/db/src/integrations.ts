...
export async function connectSlackIntegration(input: {
  ...
  if (!integration) {
    throw new Error("Could not persist opencompany Slack integration.");
  }
...
export async function connectSlackBotIntegration(input: {
  ...
  if ("batch" in db) {
    const [integrationRows, credentials] = await db.batch([
      db
        .insert(integrations)
    ...
    if (!integration || !credentials[0]) {
      throw new Error("Could not persist opencompany Slack bot integration and credential.");
    }
...
export async function connectLinearIngestIntegration(input: {
  ...
  if (!integration) {
    throw new Error("Could not persist opencompany Linear integration.");
  }
...
export async function connectHubspotIntegration(input: {
  ...
  if (!integration) {
    throw new Error("Could not persist opencompany HubSpot integration.");
  }
...
export async function applyIntegrationCapabilityMode(input: {
