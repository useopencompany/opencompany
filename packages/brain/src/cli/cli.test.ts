...
beforeEach(async () => {
  root = await mkdtemp(path.join(os.tmpdir(), "opencompany-brain-cli-"));
});
...
describe("opencompany-brain cli", () => {
  it("validates unknown options", () => {
    ...
    expect(HELP).toContain("opencompany-brain <command>");
    expect(HELP).toContain("help");
    expect(HELP).toContain("doctor");
    expect(commandHelp("create")).toContain("Usage: opencompany-brain create");
  });
  ...
  it("prints global and command-specific help", async () => {
    await expect(run(["help"])).resolves.toMatchObject({
      exitCode: 0,
      stdout: expect.stringContaining("opencompany-brain - folder-first personal brain CLI"),
    });
    await expect(run(["help", "create"])).resolves.toMatchObject({
      exitCode: 0,
      stdout: expect.stringContaining("Usage: opencompany-brain create"),
    });
  ...
  it("includes help for unknown commands, unknown options, and wrong invocation", async () => {
    ...
    await expect(run(["wat"])).resolves.toMatchObject({
      stderr: expect.stringContaining("Usage: opencompany-brain <command>"),
    });
    ...
    await expect(run(["query", "--wat"])).resolves.toMatchObject({
      stderr: expect.stringContaining("Usage: opencompany-brain query"),
    });
    ...
    await expect(run(["get", "--root", root])).resolves.toMatchObject({
      stderr: expect.stringContaining("Usage: opencompany-brain get"),
    });
    ...
    await expect(
      ...
      exitCode: 1,
      stderr: expect.stringContaining('Unsupported opencompany Brain entity type "slack"'),
    });
    await expect(
      run([
        "append-evidence",
  ...
  it("does not store the document title as the first compiled-truth heading", async () => {
    await expect(
      run([
        ...
        "Acme",
        "--truth",
        "# Acme\n\nAcme evaluates opencompany Brain.",
      ]),
    ).resolves.toMatchObject({ exitCode: 0 });
    ...
    await expect(run(["get", "--root", root, "acme", "--section", "truth"])).resolves.toMatchObject(
      {
        exitCode: 0,
        stdout: "Acme evaluates opencompany Brain.\n",
      },
    );
  ...
  it("updates title, type, and status through set", async () => {
    ...
    await expect(
      ...
      exitCode: 1,
      stderr: expect.stringContaining("Unsupported opencompany Brain entity type"),
    });
    await expect(
  ...
  it("promotes pointer-backed tracker knowledge without inventing evidence", async () => {
    ...
    await expect(
      run(["set", "--root", root, "opencompany-brain", "--status", "active"]),
    ).resolves.toMatchObject({ exitCode: 0 });
  ...
  it("runs the generated bundle under node from a temp brain root", async () => {
    const bundlePath = path.join(root, "opencompany-brain.mjs");
