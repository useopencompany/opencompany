...
describe("opencompany Brain inline links", () => {
  ...
  it("rejects invalid formatted targets and labels", () => {
    expect(() => formatBrainPageLink("Bad Id")).toThrow(
      "Invalid opencompany Brain page link target.",
    );
    expect(() => formatBrainEvidenceLink("seed")).toThrow(
      "Invalid opencompany Brain evidence link target.",
    );
    expect(() => formatBrainSourceLink("gmail:1", "bad]label")).toThrow(
      "Invalid opencompany Brain link label.",
    );
