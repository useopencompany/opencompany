...
import {
  ...
  chatScreenshotUrl,
} from "./chat-screenshot-storage";
import type { BrowserToolOutput } from "./chat-ui";
  ...
  const endActiveProfile = async () => {
    ...
    if (!activeProfileEndPromise) {
      ...
      activeProfileEndPromise = input
        ...
        .catch((error) => {
          logger.warn("opencompany browser profile session cleanup failed", {
  ...
  const executeBrowserTool = async ({
    ...
    if (name === "browser_screenshot" && screenshotFilename && screenshotPath) {
      try {
        ...
        logger.warn("opencompany chat browser screenshot storage failed", {
