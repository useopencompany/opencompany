...
import { BRAIN_CAPTURE_FOLDER } from "./brain-capture";
import type { SaveToBrainToolOutput, StoredChatMessage } from "./chat-ui";

export type ChatAttachmentCaptureDependencies = {
...
export async function saveChatAttachmentsToBrain(
  input: {
    brainRef: string;
    actorId: string;
  ...
  dependencies: ChatAttachmentCaptureDependencies,
  ...
  if (dependencies.wakeIngest) {
    dependencies.wakeIngest().catch((error) => {
      console.warn("opencompany chat attachment capture failed to wake the ingest worker.", {
