...
import { XAccessAuthError, xAccountApiCall } from "../integrations/x-access-token";
...
function postTweetAction(connections: readonly XAccountConnection[]): ResolvedAction {
  const multipleAccounts = connections.length > 1;
  const params: ResolvedAction["params"] = multipleAccounts
    ? {
        type: "object",
...
async function loadXAccountConnections(userWorkosId: string): Promise<XAccountConnection[]> {
  ...
  return rows
    ...
    .map((row) => ({
      ...
      // connectionLabel stores "@username" (see connectXAccountIntegration).
...
function resolvePostTargets(
  ...
  const targets = posts.map((post, index) => {
    if (!post || typeof post !== "object" || Array.isArray(post)) {
      throw new ActionInvalidParamsError(`"posts[${index}]" must be an object.`);
    }
...
function assertPostTextLength(text: string, field: string) {
  if (text.length > MAX_TWEET_CHARS) {
    throw new ActionInvalidParamsError(`"${field}" exceeds ${MAX_TWEET_CHARS} characters.`);
  }
}
