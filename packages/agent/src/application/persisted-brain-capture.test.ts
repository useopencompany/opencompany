import type { CodexBrainCaptureGatewayRequest } from "@opencompany/agent-runtime";
import { describe, expect, it, vi } from "vitest";
import type { BrainCaptureServiceDependencies } from "./brain-capture";
import { executePersistedBrainCapture } from "./persisted-brain-capture";
...
function executeBrainCapture(input: {
  ...
  dependencies?: Partial<BrainCaptureServiceDependencies>;
}) {
  return executePersistedBrainCapture({
...
describe("executePersistedBrainCapture", () => {
  it("uses the pinned Brain and the existing capture-first pipeline", async () => {
    const capture = vi.fn<BrainCaptureServiceDependencies["capture"]>(async () => ({
      ok: true as const,
