...
export type GoogleDriveMcpTicketPayload = {
  v: 1;
...
export function createGoogleDriveMcpTicket(input: {
  userWorkosId: string;
  workspaceId: string;
  integrationId: string;
  registrationId: string;
  ...
  } satisfies GoogleDriveMcpTicketPayload;
...
export function verifyGoogleDriveMcpTicket(input: {
  ...
  secret: string;
  now?: number;
}): GoogleDriveMcpTicketPayload | null {
  ...
  try {
    ...
    ) as Partial<GoogleDriveMcpTicketPayload>;
    if (
    ...
    return value as GoogleDriveMcpTicketPayload;
  } catch {
    return null;
  }
}

function validOperation(value: unknown): value is GoogleDriveMcpOperation {
...
function sign(value: string, secret: string) {
  return createHmac("sha256", secret)
    .update("opencompany-google-drive-mcp-ticket")
    .update("\0")
    .update(value)
    .digest("base64url");
}
