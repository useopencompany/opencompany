...
import type { JamieProviderState } from "../integration-state";
import {
...
export async function getJamieIntegrationState(
  workspaceId: string,
  db: DbLike = getDb(),
): Promise<JamieProviderState> {
  const [row] = await db
    .select({
      id: integrations.id,
...
export async function loadJamieWebhookContext(
  ...
  return loadJamieWebhookContextForIntegration(integration, db);
}
...
export async function loadJamieWebhookContextForApiKey(
  ...
  if (directIntegration) {
    return loadJamieWebhookContextForIntegration(directIntegration, db);
  }
  ...
  for (const integration of integrationRows) {
    const context = await loadJamieWebhookContextForIntegration(integration, db);
    if (!context) continue;
  ...
  try {
    ...
    console.warn("[opencompany-jamie] Failed to backfill Jamie API key lookup hash", {
...
async function loadJamieWebhookContextForIntegration(
  integration: {
    id: string;
    userWorkosId: string;
  },
...
export async function markJamieWebhookConnected(
  input: {
    integrationId: string;
    userWorkosId: string;
    now?: Date;
