...
// Matches the loose injection convention used by the other shared shared modules:
...
export async function syncGitHubIntegrationRepositories(
  input: {
    userWorkosId: string;
    workspaceId: string;
    installationId: string;
  ...
  if (!integration) {
    throw new Error("Could not persist opencompany GitHub integration.");
  }
...
function requiredEnv(name: string) {
  ...
  if (!value) throw new Error(`${name} is required for opencompany GitHub integration.`);
  return value;
}
