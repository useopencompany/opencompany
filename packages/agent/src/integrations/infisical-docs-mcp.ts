...
// availability so the plugin and the sandbox CLI share one visible connection lifecycle, but its
// encrypted CLI auth bundle is deliberately never loaded or forwarded to the remote endpoint.
...
export async function loadInfisicalDocsMcpWorkerConnection(
  ...
  return { ok: true, integrationId: integrationId(metadata) };
}
