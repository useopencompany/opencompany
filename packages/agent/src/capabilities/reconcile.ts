...
export async function reconcileCapabilities(limit = 100, deps: { db?: any } = {}) {
  const expiredApprovals = await expirePendingCapabilityApprovals({ db: deps.db });
  ...
  const candidates = await listUnsettledCapabilityRuns({ limit, db: deps.db });
  let settled = 0;
  let pending = 0;
  let failed = 0;
