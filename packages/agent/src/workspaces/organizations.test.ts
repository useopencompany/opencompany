...
import { ensureWorkspaceOrganization } from "./organizations";
...
describe("ensureWorkspaceOrganization", () => {
  ...
  it("creates a WorkOS organization with the opencompany workspace id as externalId", async () => {
    ...
    await expect(ensureWorkspaceOrganization(workspace(), { workos, db })).resolves.toBe("org_new");
  ...
  it("links an existing WorkOS organization found by externalId", async () => {
    ...
    await expect(ensureWorkspaceOrganization(workspace(), { workos, db })).resolves.toBe(
      "org_existing",
    );
