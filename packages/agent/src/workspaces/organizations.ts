...
import type { Workspace } from "@opencompany/db/product-schema";
import {
...
export async function ensureWorkspaceOrganization(
  ...
  if (existing) {
    await syncWorkspaceMembersToOrganization(workos, workspace.id, existing.id, db);
  ...
  try {
    ...
    await syncWorkspaceMembersToOrganization(workos, workspace.id, organization.id, db);
    await setWorkspaceOrganizationId(
      {
        workspaceId: workspace.id,
    ...
    if (isWorkOSConflict(error)) {
      ...
      if (conflicted) {
        await syncWorkspaceMembersToOrganization(workos, workspace.id, conflicted.id, db);
        await setWorkspaceOrganizationId(
          {
            workspaceId: workspace.id,
    ...
    if (organizationId) {
      try {
        ...
        console.error("[opencompany] Failed to clean up WorkOS organization", cleanupError);
      }
    }
    throw error;
  }
...
export async function ensureWorkspaceOrganizationsForEntries<T extends { workspace: Workspace }>(
  ...
  for (const entry of entries) {
    try {
      const workosOrganizationId = await ensureWorkspaceOrganization(entry.workspace, deps);
      next.push({
        ...entry,
        workspace: { ...entry.workspace, workosOrganizationId },
      });
    } catch (error) {
      console.error("[opencompany] Failed to ensure WorkOS organization for workspace", {
...
async function findWorkspaceOrganization(workos: WorkOSClientLike, workspaceId: string) {
  try {
...
async function createWorkspaceOrganization(workos: WorkOSClientLike, workspace: Workspace) {
...
async function syncWorkspaceMembersToOrganization(
  ...
  const members = await listWorkspaceMembers(workspaceId, { db });
